...
from .appdaemon_stub import AppDaemonHassStub as ADHass
...
CONFIG_ITEMS = [
    {
        ...
        "release_url": "https://github.com/springfall2008/batpred/releases/tag/"
        + THIS_VERSION,
...
class Inverter:
    def self_test(self, minutes_now):
        self.base.log(
            "======= INVERTER CONTROL SELF TEST START - REST={} ========".format(
                self.rest_api
            )
        )
    ...
    def __init__(self, base, id=0, quiet=False):
        ...
        self.reserve_percent = self.base.get_arg(
            "battery_min_soc", default=4.0, index=self.id
        )
        self.reserve_percent_current = self.base.get_arg(
            "battery_min_soc", default=4.0, index=self.id
        )
        ...
        self.inv_output_charge_control = INVERTER_DEF[self.inverter_type][
            "output_charge_control"
        ]
        self.inv_has_charge_enable_time = INVERTER_DEF[self.inverter_type][
            "has_charge_enable_time"
        ]
        self.inv_has_discharge_enable_time = INVERTER_DEF[self.inverter_type][
        ...
        self.inv_charge_time_format = INVERTER_DEF[self.inverter_type][
            "charge_time_format"
        ]
        self.inv_charge_time_entity_is_option = INVERTER_DEF[self.inverter_type][
            "charge_time_entity_is_option"
        ]
        self.inv_clock_time_format = INVERTER_DEF[self.inverter_type][
            "clock_time_format"
        ]
        self.inv_soc_units = INVERTER_DEF[self.inverter_type]["soc_units"]
        self.inv_time_button_press = INVERTER_DEF[self.inverter_type][
            "time_button_press"
        ]
        self.inv_support_discharge_freeze = INVERTER_DEF[self.inverter_type][
            "support_discharge_freeze"
        ]
        self.inv_has_ge_inverter_mode = INVERTER_DEF[self.inverter_type][
            "has_ge_inverter_mode"
        ]
        self.inv_num_load_entities = INVERTER_DEF[self.inverter_type][
            "num_load_entities"
        ]
        self.inv_write_and_poll_sleep = INVERTER_DEF[self.inverter_type][
        ...
        if self.inverter_type == "GE":
            self.rest_api = self.base.get_arg(
                "givtcp_rest", None, indirect=False, index=self.id
            )
            if self.rest_api:
                if not quiet:
                    self.base.log(
                        "Inverter {} using Rest API {}".format(self.id, self.rest_api)
                    )
        ...
        if self.rest_data and ("Invertor_Details" in self.rest_data):
            ...
            if "raw" in self.rest_data:
                self.nominal_capacity = (
                    float(self.rest_data["raw"]["invertor"]["battery_nominal_capacity"])
                    / 19.53125
            ...
            self.soc_max = (
                self.base.get_arg("soc_max", default=10.0, index=self.id)
            ...
            if self.inverter_type == "GE":
                ...
                self.battery_rate_max_raw = self.base.get_arg(
                    "battery_rate_max", index=self.id, default=2600.0
                )
            else:
        ...
        if self.soc_max <= 0:
            self.base.log(
                "ERROR: Reported battery size from REST is {}, but it must be >0".format(
                    self.soc_max
                )
            )
            raise ValueError
        ...
        self.battery_rate_max_charge = (
            min(
                self.base.get_arg(
                    "inverter_limit_charge", self.battery_rate_max_raw, index=self.id
                ),
        ...
        self.battery_rate_max_discharge = (
            min(
                self.base.get_arg(
                    "inverter_limit_discharge", self.battery_rate_max_raw, index=self.id
                ),
                self.battery_rate_max_raw,
            )
            / 60.0
        ...
        self.battery_rate_max_charge_scaled = (
            self.battery_rate_max_charge * self.base.battery_rate_max_scaling
        )
        self.battery_rate_max_discharge_scaled = (
            self.battery_rate_max_discharge * self.base.battery_rate_max_scaling
        )
        self.battery_rate_min = (
            min(
                self.base.get_arg("inverter_battery_rate_min", 0, index=self.id),
                self.battery_rate_max_raw,
            )
            / 60.0
            / 1000.0
        ...
        if ivtime:
            try:
                ...
                try:
                    ...
                    try:
                        tz = pytz.timezone(
                            self.base.get_arg("timezone", "Europe/London")
                        )
                        self.inverter_time = tz.localize(
                            datetime.strptime(ivtime, self.inv_clock_time_format)
                        )
                    except (ValueError, TypeError):
                        self.base.log(
        ...
        if self.rest_data:
            self.reserve_percent_current = float(
                self.rest_data["Control"]["Battery_Power_Reserve"]
            )
        else:
        ...
        self.reserve_current = self.base.dp2(
            self.soc_max * self.reserve_percent_current / 100.0
        )

        # Get the expected minimum reserve value
        battery_min_soc = self.base.get_arg(
            "battery_min_soc", default=4.0, index=self.id
        )
        ...
        self.base.log(
            f"Reserve min: {self.reserve_min}% Battery_min:{battery_min_soc}%"
        )
        ...
        if "inverter_limit" in self.base.args:
            self.inverter_limit = self.base.get_arg(
                "inverter_limit", self.inverter_limit, index=self.id
            ) / (1000 * 60.0)
        if "export_limit" in self.base.args:
            self.export_limit = self.base.get_arg(
                "export_limit", self.inverter_limit, index=self.id
            ) / (1000 * 60.0)
        ...
        if not self.inv_has_charge_enable_time:
            self.base.args["scheduled_charge_enable"] = self.create_entity(
                "scheduled_charge_enable", False
            )

        if not self.inv_has_discharge_enable_time:
            self.base.args["scheduled_discharge_enable"] = self.create_entity(
                "scheduled_discharge_enable", False
            )

        if not self.inv_has_reserve_soc:
            self.base.args["reserve"] = self.create_entity(
                "reserve", self.reserve, device_class="battery", uom="%"
            )

        if not self.inv_has_target_soc:
            self.base.args["charge_limit"] = self.create_entity(
                "charge_limit", 100, device_class="battery", uom="%"
            )

        if self.inv_output_charge_control != "power":
            self.base.args["charge_rate"] = self.create_entity(
                "charge_rate",
                int(self.battery_rate_max_charge * 60 * 1000),
                uom="W",
                device_class="power",
            )
            self.base.args["discharge_rate"] = self.create_entity(
                "discharge_rate",
                int(self.battery_rate_max_discharge * 60 * 1000),
                uom="W",
                device_class="power",
            )
        ...
        if self.inv_charge_time_format != "HH:MM:SS":
            for x in ["charge", "discharge"]:
                for y in ["start", "end"]:
                    self.base.args[f"{x}_{y}_time"] = self.create_entity(
                        f"{x}_{y}_time", "23:59:00"
                    )
    ...
    def update_status(self, minutes_now, quiet=False):
        ...
        if self.rest_data:
            self.charge_enable_time = (
                self.rest_data["Control"]["Enable_Charge_Schedule"] == "enable"
            )
            self.discharge_enable_time = (
                self.rest_data["Control"]["Enable_Discharge_Schedule"] == "enable"
            )
            self.charge_rate_now = (
                self.rest_data["Control"]["Battery_Charge_Rate"] / 1000.0 / 60.0
            )
            self.discharge_rate_now = (
                self.rest_data["Control"]["Battery_Discharge_Rate"] / 1000.0 / 60.0
            )
        else:
            self.charge_enable_time = (
                self.base.get_arg("scheduled_charge_enable", "on", index=self.id)
                == "on"
            )
            self.discharge_enable_time = (
                self.base.get_arg("scheduled_discharge_enable", "off", index=self.id)
                == "on"
            )
            self.charge_rate_now = (
            ...
            self.discharge_rate_now = (
                self.base.get_arg("discharge_rate", index=self.id, default=2600.0)
                / 1000.0
                / 60.0
            )

        # Scale charge and discharge rates with battery scaling
        self.charge_rate_now = max(
            self.charge_rate_now * self.base.battery_rate_max_scaling,
            self.battery_rate_min,
        )
        self.discharge_rate_now = max(
            self.discharge_rate_now * self.base.battery_rate_max_scaling,
            self.battery_rate_min,
        )
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                self.soc_kw = (
                    self.rest_data["Power"]["Power"]["SOC_kWh"]
                    * self.base.battery_scaling
                )
            else:
                if "soc_percent" in self.base.args:
                    ...
                    self.soc_kw = (
                        self.base.get_arg("soc_kw", default=0.0, index=self.id)
                        * self.base.battery_scaling
                    )
        ...
        if self.rest_data and ("Power" in self.rest_data):
            ...
            self.battery_power = self.base.get_arg(
                "battery_power", default=0.0, index=self.id
            )
            self.pv_power = self.base.get_arg("pv_power", default=0.0, index=self.id)
            self.load_power = self.base.get_arg(
                "load_power", default=0.0, index=self.id
            )

            for i in range(1, self.inv_num_load_entities):
                self.load_power += self.base.get_arg(
                    f"load_power_{i}", default=0.0, index=self.id
                )

        self.battery_voltage = self.base.get_arg(
            "battery_voltage", default=52.0, index=self.id
        )
        ...
        if self.charge_enable_time:
            ...
            if SIMULATE:
                charge_start_time = datetime.strptime(
                    self.base.sim_charge_start_time, "%H:%M:%S"
                )
                charge_end_time = datetime.strptime(
                    self.base.sim_charge_end_time, "%H:%M:%S"
                )
            else:
                if self.rest_data:
                    charge_start_time = datetime.strptime(
                        self.rest_data["Timeslots"]["Charge_start_time_slot_1"],
                        "%H:%M:%S",
                    )
                    charge_end_time = datetime.strptime(
                        self.rest_data["Timeslots"]["Charge_end_time_slot_1"],
                        "%H:%M:%S",
                    )
                elif "charge_start_time" in self.base.args:
                    charge_start_time = datetime.strptime(
                        self.base.get_arg("charge_start_time", index=self.id),
                        "%H:%M:%S",
                    )
                    charge_end_time = datetime.strptime(
            ...
            # Reverse clock skew
            charge_start_time -= timedelta(
                seconds=self.base.inverter_clock_skew_start * 60
            )
            ...
            self.charge_start_time_minutes = (
                charge_start_time.hour * 60 + charge_start_time.minute
            )
            self.charge_end_time_minutes = (
                charge_end_time.hour * 60 + charge_end_time.minute
            )
        ...
        if not quiet:
            self.base.log(
                "Inverter {} scheduled charge enable is {}".format(
                    self.id, self.charge_enable_time
                )
            )

        if self.charge_enable_time:
            minute = max(
                0, self.charge_start_time_minutes
            )  # Max is here is start could be before midnight now
        ...
        if not quiet:
            self.base.log(
                "Inverter {} charge windows currently {}".format(
                    self.id, self.charge_window
                )
            )
        ...
        if self.charge_enable_time:
            if self.rest_data:
                self.current_charge_limit = float(
                    self.rest_data["Control"]["Target_SOC"]
                )
            else:
                self.current_charge_limit = float(
                    self.base.get_arg("charge_limit", index=self.id, default=100.0)
                )
        else:
            self.current_charge_limit = 0.0
        ...
        if self.rest_data:
            ...
            discharge_end = datetime.strptime(
                self.rest_data["Timeslots"]["Discharge_end_time_slot_1"], "%H:%M:%S"
            )
            ...
            discharge_end = datetime.strptime(
                self.base.get_arg("discharge_end_time", index=self.id), "%H:%M:%S"
            )
        else:
            self.base.log(
        ...
        # Reverse clock skew
        discharge_start -= timedelta(
            seconds=self.base.inverter_clock_skew_discharge_start * 60
        )
        discharge_end -= timedelta(
            seconds=self.base.inverter_clock_skew_discharge_end * 60
        )

        # Compute discharge window minutes start/end just for the next discharge window
        self.discharge_start_time_minutes = (
            discharge_start.hour * 60 + discharge_start.minute
        )
        ...
        if not quiet:
            self.base.log(
                "Inverter {} scheduled discharge enable is {}".format(
                    self.id, self.discharge_enable_time
                )
            )
        ...
        if True:
            minute = max(
                0, self.discharge_start_time_minutes
            )  # Max is here is start could be before midnight now
        ...
        if self.discharge_enable_time:
            ...
            self.discharge_limits = [
                100.0 for i in range(0, len(self.discharge_window))
            ]

        if not quiet:
            self.base.log(
                "Inverter {} discharge windows currently {}".format(
                    self.id, self.discharge_window
                )
            )
    ...
    def mimic_target_soc(self, current_charge_limit):
        ...
        if self.soc_percent >= float(current_charge_limit):
            ...
            self.base.log(
                f"Current SOC {self.soc_percent}% is greater than Target SOC {current_charge_limit}. Grid Charge disabled."
            )
        else:
            ...
            if self.inv_output_charge_control == "current":
                self.set_current_from_power(
                    "charge", self.battery_rate_max_charge * 60 * 1000
                )
            self.base.log(
    ...
    def adjust_reserve(self, reserve):
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_reserve = self.base.get_arg(
                    "reserve", index=self.id, default=0.0
                )
        ...
        if current_reserve != reserve:
            self.base.log(
                "Inverter {} Current Reserve is {} % and new target is {} %".format(
                    self.id, current_reserve, reserve
                )
            )
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_soc = self.base.get_entity(
                        self.base.get_arg("reserve", indirect=False, index=self.id)
                    )
                ...
                self.base.record_status(
                    "Inverter {} set reserve to {}".format(self.id, reserve)
                )
            self.mqtt_message(topic="set/reserve", payload=reserve)
        else:
            self.base.log(
                "Inverter {} Current reserve is {} already at target".format(
                    self.id, current_reserve
                )
            )
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_rate = self.base.get_arg(
                    "charge_rate", index=self.id, default=2600.0
                )

        if abs(current_rate - new_rate) > 100:
            self.base.log(
                "Inverter {} current charge rate is {} and new target is {}".format(
                    self.id, current_rate, new_rate
                )
            )
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    self.write_and_poll_value(
                        "charge_rate", entity, new_rate, fuzzy=100
                    )
                    if self.inv_output_charge_control == "current":
                ...
                if notify and self.base.set_inverter_notify:
                    self.base.call_notify(
                        "Predbat: Inverter {} charge rate changes to {} at {}".format(
                            self.id, new_rate, self.base.time_now_str()
                        )
                    )
            if notify:
                self.base.record_status(
                    "Inverter {} charge rate changed to {}".format(self.id, new_rate)
                )
        ...
        *If the inverter uses current rather than power we create a dummy entity for the power anyway but also write to the current entity
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                current_rate = self.base.get_arg(
                    "discharge_rate", index=self.id, default=2600.0
                )

        if abs(current_rate - new_rate) > 100:
            self.base.log(
                "Inverter {} current discharge rate is {} and new target is {}".format(
                    self.id, current_rate, new_rate
                )
            )
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity = self.base.get_entity(
                        self.base.get_arg(
                            "discharge_rate", indirect=False, index=self.id
                        )
                    )
                    self.write_and_poll_value(
                        "discharge_rate", entity, new_rate, fuzzy=100
                    )
                ...
                if notify and self.base.set_inverter_notify:
                    self.base.call_notify(
                        "Predbat: Inverter {} discharge rate changes to {} at {}".format(
                            self.id, new_rate, self.base.time_now_str()
                        )
                    )
            if notify:
                self.base.record_status(
                    "Inverter {} discharge rate changed to {}".format(self.id, new_rate)
                )
    ...
    def adjust_battery_target(self, soc):
        ...
        if current_soc != soc:
            self.base.log(
                "Inverter {} Current charge limit is {} % and new target is {} %".format(
                    self.id, current_soc, soc
                )
            )
            ...
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_soc = self.base.get_entity(
                        self.base.get_arg("charge_limit", indirect=False, index=self.id)
                    )
            ...
            self.base.log(
                "Inverter {} Current Target SOC is {} already at target".format(
                    self.id, current_soc
                )
            )
    ...
    def write_and_poll_value(self, name, entity, new_value, fuzzy=0):
        ...
        if retry == 0:
            self.base.log(
                f"Inverter {self.id} No write needed for {name}: {new_value} == {current_state}"
            )
            return True
        elif matched:
            self.base.log(
                f"Inverter {self.id} Wrote {new_value} to {name}, successfully now {current_state}"
            )
            return True
        else:
            self.base.log(
            ...
            self.base.record_status(
                f"Warn - Inverter {self.id} write to {name} failed", had_errors=True
            )
            return False
    ...
    def write_and_poll_option(self, name, entity, new_value):
        ...
        for retry in range(0, 6):
            ...
            if old_value == new_value:
                self.base.log(
                    "Inverter {} Wrote {} to {} successfully".format(
                        self.id, name, new_value
                    )
                )
                return True
        self.base.log(
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                ...
                if changed_start_end and not self.rest_data:
                    ...
                    self.base.log(
                        "Sleeping (workaround) as start/end of discharge window was just adjusted"
                    )
                    time.sleep(30)
        ...
        if old_inverter_mode != new_inverter_mode:
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity = self.base.get_entity(
                        self.base.get_arg(
                            "inverter_mode", indirect=False, index=self.id
                        )
                    )
                    if self.inv_has_ge_inverter_mode:
                        self.write_and_poll_option(
                            "inverter_mode", entity, new_inverter_mode
                        )
                    else:
                        self.write_and_poll_value(
                            "inverter_mode", entity, new_inverter_mode
            ...
            self.base.record_status(
                "Inverter {} Set discharge mode to {}".format(
                    self.id, new_inverter_mode
                )
            )
            self.base.log(
                "Inverter {} set force discharge to {}".format(self.id, force_discharge)
            )

    def adjust_force_discharge(
        self, force_discharge, new_start_time=None, new_end_time=None
    ):
        """
        ...
        if new_start and new_start != old_start:
            self.base.log(
                "Inverter {} set new start time to {}".format(self.id, new_start)
            )
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_discharge_start_time = self.base.get_entity(
                        self.base.get_arg(
                            "discharge_start_time", indirect=False, index=self.id
                        )
                    )
                    if self.inv_charge_time_entity_is_option:
                        self.write_and_poll_option(
                            "discharge_start_time",
                            entity_discharge_start_time,
                            new_start,
                        )
                    else:
                        self.write_and_poll_value(
                    ...
                    if self.inv_charge_time_format == "H M":
                        ...
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "discharge_start_hour", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "discharge_start_hour", entity, int(new_start[:2])
                        )
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "discharge_start_minute", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "discharge_start_minute", entity, int(new_start[3:5])
                        )
                else:
                    self.log(
        ...
        if new_end and new_end != old_end:
            self.base.log(
                "Inverter {} Set new end time to {} was {}".format(
                    self.id, new_end, old_end
                )
            )
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_discharge_end_time = self.base.get_entity(
                        self.base.get_arg(
                            "discharge_end_time", indirect=False, index=self.id
                        )
                    )
                    if self.inv_charge_time_entity_is_option:
                        self.write_and_poll_option(
                            "discharge_end_time", entity_discharge_end_time, new_end
                        )
                        # If the inverter uses hours and minutes then write to these entities too
                    else:
                        self.write_and_poll_value(
                    ...
                    if self.inv_charge_time_format == "H M":
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "discharge_end_hour", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "discharge_end_hour", entity, int(new_end[:2])
                        )
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "discharge_end_minute", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "discharge_end_minute", entity, int(new_end[3:5])
                        )
                else:
                    self.log(
        ...
        if (new_end != old_end) or (new_start != old_start):
            ...
            if self.inv_time_button_press:
                entity_id = self.base.get_arg(
                    "charge_discharge_update_button", indirect=False, index=self.id
                )
        ...
        if (
            ...
            and new_start
            and new_end
            and ((new_start != old_start) or (new_end != old_end))
        ):
            changed_start_end = True
            if not SIMULATE:
        ...
        if force_discharge:
            self.adjust_inverter_mode(
                force_discharge, changed_start_end=changed_start_end
            )

        # Notify
        if changed_start_end:
            self.base.record_status(
                "Inverter {} set discharge slot to {} - {}".format(
                    self.id, new_start, new_end
                )
            )
            if self.base.set_inverter_notify:
                self.base.call_notify(
    ...
    def disable_charge_window(self, notify=True):
        ...
        if SIMULATE:
            ...
            if self.rest_data:
                old_charge_schedule_enable = self.rest_data["Control"][
                    "Enable_Charge_Schedule"
                ]
            else:
                old_charge_schedule_enable = self.base.get_arg(
                    "scheduled_charge_enable", "on", index=self.id
                )
        ...
        if old_charge_schedule_enable == "on" or old_charge_schedule_enable == "enable":
            if not SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity = self.base.get_entity(
                        self.base.get_arg(
                            "scheduled_charge_enable", indirect=False, index=self.id
                        )
                    )
                    ...
                    if not self.inv_has_charge_enable_time and (
                        self.inv_output_charge_control == "current"
                    ):
            ...
            if notify:
                self.base.record_status(
                    "Inverter {} Turned off scheduled charge".format(self.id)
                )
    ...
    def alt_charge_discharge_enable(self, direction, enable, grid=True, timed=False):
        ...
        if not grid and not timed:
            self.base.log(
                "WARN: Unable to set Solis Energy Controls Switch: both Grid and Timed are False"
            )
            return False
        ...
        if self.inverter_type == "GS":
            ...
            entity_id = self.base.get_arg(
                "energy_control_switch", indirect=False, index=self.id
            )
            entity = self.base.get_entity(entity_id)
            ...
            if switch > 0:
                ...
                if new_switch != switch:
                    ...
                    new_mode = {SOLAX_SOLIS_MODES[x]: x for x in SOLAX_SOLIS_MODES}[
                        new_switch
                    ]

                    self.write_and_poll_option(
                        name=entity_id, entity=entity, new_value=new_mode
                    )
                    return True
                else:
                    self.base.log(
        ...
        if direction == "charge" and enable:
            self.mqtt_message(
                "set/charge", payload=int(self.battery_rate_max_charge * 60 * 1000)
            )
        elif direction == "discharge" and enable:
            self.mqtt_message(
                "set/discharge",
                payload=int(self.battery_rate_max_discharge * 60 * 1000),
            )
        else:
    ...
    def mqtt_message(self, topic, payload):
        ...
        if self.inv_has_mqtt_api:
            self.base.call_service(
                ...
                qos=1,
                retain=True,
                topic=(self.inv_mqtt_topic + "/" + topic),
                payload=payload,
            )
    ...
    def enable_charge_discharge_with_time_current(self, direction, enable):
        ...
        if enable:
            power = self.base.get_arg(
                f"{direction}_rate", index=self.id, default=2600.0
            )
            ...
            for x in ["start", "end"]:
                for y in ["hour", "minute"]:
                    ...
                    entity = self.base.get_entity(
                        self.base.get_arg(name, indirect=False, index=self.id)
                    )
    ...
    def set_current_from_power(self, direction, power):
        ...
        entity = self.base.get_entity(
            self.base.get_arg(
                f"timed_{direction}_current", indirect=False, index=self.id
            )
        )
        self.write_and_poll_value(
            f"timed_{direction}_current", entity, new_current, fuzzy=1
        )
    ...
    def adjust_charge_window(self, charge_start_time, charge_end_time, minutes_now):
            ...
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_start = self.base.get_entity(
                        self.base.get_arg(
                            "charge_start_time", indirect=False, index=self.id
                        )
                    )
                    if self.inv_charge_time_entity_is_option:
                        self.write_and_poll_option(
                            "charge_start_time", entity_start, new_start
                        )
                    else:
                        self.write_and_poll_value(
                            "charge_start_time", entity_start, new_start
                    ...
                    if self.inv_charge_time_format == "H M":
                        ...
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "charge_start_hour", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "charge_start_hour", entity, int(new_start[:2])
                        )
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "charge_start_minute", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "charge_start_minute", entity, int(new_start[3:5])
                        )
                else:
                    self.log(
            ...
            if SIMULATE:
                ...
                if self.rest_data:
                    ...
                    entity_end = self.base.get_entity(
                        self.base.get_arg(
                            "charge_end_time", indirect=False, index=self.id
                        )
                    )
                    if self.inv_charge_time_entity_is_option:
                        self.write_and_poll_option(
                            "charge_end_time", entity_end, new_end
                        )
                    else:
                        self.write_and_poll_value(
                            "charge_end_time", entity_end, new_end
                    ...
                    if self.inv_charge_time_format == "H M":
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "charge_end_hour", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "charge_end_hour", entity, int(new_end[:2])
                        )
                        entity = self.base.get_entity(
                            self.base.get_arg(
                                "charge_end_minute", indirect=False, index=self.id
                            )
                        )
                        self.write_and_poll_value(
                            "charge_end_minute", entity, int(new_end[3:5])
                        )
                else:
                    self.log(
        ...
        if new_start != old_start or new_end != old_end:
            ...
            self.base.record_status(
                "Inverter {} Charge window change to: {} - {}".format(
                    self.id, new_start, new_end
                )
            )
            self.base.log(
        ...
        if (
            ...
            if not SIMULATE:
                ...
                if (
                    old_charge_schedule_enable == "off"
                    or old_charge_schedule_enable == "disable"
                    and self.base.set_inverter_notify
                ):
                    self.base.call_notify(
            ...
            self.base.record_status(
                "Inverter {} Turned on charge enable".format(self.id)
            )
    ...
    def press_and_poll_button(self, entity_id):
        for retry in range(0, 6):
            ...
            if (
                pytz.timezone("UTC").localize(datetime.now()) - time_pressed
            ).seconds < 10:
                self.base.log(
                ...
                return True
        self.base.log(
            f"WARN: Inverter {self.id} Trying to press {entity_id} didn't complete"
        )
        self.base.record_status(
            f"Warn: Inverter {self.id} Trying to press {entity_id} didn't complete"
        )
    ...
    def rest_readData(self, api="readData"):
        ...
        if r and (r.status_code == 200):
            ...
            if "Control" in json:
                ...
                self.base.log(
                    "WARN: Inverter {} read bad REST data from {} - REST will be disabled".format(
                        self.id, url
                    )
                )
                self.base.record_status(
                    "Inverter {} read bad REST data from {} - REST will be disabled".format(
                        self.id, url
                    ),
                    had_errors=True,
                )
            ...
            self.base.log(
                "WARN: Inverter {} unable to read REST data from {} - REST will be disabled".format(
                    self.id, url
                )
            )
            self.base.record_status(
                "Inverter {} unable to read REST data from {} - REST will be disabled".format(
                    self.id, url
                ),
                had_errors=True,
            )
    ...
    def rest_setChargeTarget(self, target):
        ...
        for retry in range(0, 5):
            ...
            if float(self.rest_data["Control"]["Target_SOC"]) == target:
                self.base.log(
                    "Inverter {} charge target {} via REST successful on retry {}".format(
                        self.id, target, retry
                    )
                )
                return True

        self.base.log(
            "WARN: Inverter {} charge target {} via REST failed".format(self.id, target)
        )
        self.base.record_status(
            "Warn - Inverter {} REST failed to setChargeTarget".format(self.id),
            had_errors=True,
        )
        return False
    ...
    def rest_setChargeRate(self, rate):
        ...
        for retry in range(0, 5):
            ...
            if abs(new - rate) < 100:
                self.base.log(
                    "Inverter {} set charge rate {} via REST successful on retry {}".format(
                        self.id, rate, retry
                    )
                )
                return True
        ...
        self.base.record_status(
            "Warn - Inverter {} REST failed to setChargeRate".format(self.id),
            had_errors=True,
        )
        return False
    ...
    def rest_setDischargeRate(self, rate):
        ...
        for retry in range(0, 5):
            ...
            if abs(new - rate) < 100:
                self.base.log(
                    "Inverter {} set discharge rate {} via REST successful on retry {}".format(
                        self.id, rate, retry
                    )
                )
                return True
    ...
    def rest_setBatteryMode(self, inverter_mode):
        ...
        for retry in range(0, 5):
            ...
            if inverter_mode == self.rest_data["Control"]["Mode"]:
                self.base.log(
                    "Set inverter {} mode {} via REST successful on retry {}".format(
                        self.id, inverter_mode, retry
                    )
                )
                return True

        self.base.log(
            "WARN: Set inverter {} mode {} via REST failed".format(
                self.id, inverter_mode
            )
        )
        self.base.record_status(
            "Warn - Inverter {} REST failed to setBatteryMode".format(self.id),
            had_errors=True,
        )
        return False
    ...
    def rest_setReserve(self, target):
        ...
        for retry in range(0, 5):
            ...
            if result == target:
                self.base.log(
                    "Set inverter {} reserve {} via REST successful on retry {}".format(
                        self.id, target, retry
                    )
                )
                return True
        ...
        self.base.record_status(
            "Warn - Inverter {} REST failed to setReserve to {} got {}".format(
                self.id, target, result
            ),
            had_errors=True,
        )
        return False
    ...
    def rest_enableChargeSchedule(self, enable):
        ...
        for retry in range(0, 5):
            ...
            if new_value == enable:
                self.base.log(
                    "Set inverter {} charge schedule {} via REST successful on retry {}".format(
                        self.id, enable, retry
                    )
                )
                return True
        ...
        self.base.record_status(
            "Warn - Inverter {} REST failed to enableChargeSchedule".format(self.id),
            had_errors=True,
        )
        return False
    ...
    def rest_setChargeSlot1(self, start, finish):
        ...
        for retry in range(0, 5):
            ...
            if (
                ...
                self.base.log(
                    "Inverter {} set charge slot 1 {} via REST successful after retry {}".format(
                        self.id, data, retry
                    )
                )
                return True
        ...
        self.base.log(
            "WARN: Inverter {} set charge slot 1 {} via REST failed".format(
                self.id, data
            )
        )
        self.base.record_status(
            "Warn - Inverter {} REST failed to setChargeSlot1".format(self.id),
            had_errors=True,
        )
        return False
    ...
    def rest_setDischargeSlot1(self, start, finish):
        ...
        self.base.log(
            "WARN: Inverter {} Set discharge slot 1 {} via REST failed".format(
                self.id, data
            )
        )
        self.base.record_status(
            "Warn - Inverter {} REST failed to setDischargeSlot1".format(self.id),
            had_errors=True,
        )
        return False


class PredBat(ADHass):
    """
    ...
    def resolve_arg(
        ...
        if isinstance(value, list) and (index is not None):
            if index < len(value):
                ...
                self.log(
                    "WARN: Out of range index {} within item {} value {}".format(
                        index, arg, value
                    )
                )
                value = None
            ...
            if combine:
                ...
                for item in value:
                    ...
                    try:
                        ...
                        self.log(
                            "WARN: Return bad value {} from {} arg {}".format(
                                got, item, arg
                            )
                        )
                        self.record_status(
                ...
                final = []
                for item in value:
                    item = self.resolve_arg(
                        arg, item, default=default, indirect=indirect
                    )
                    final.append(item)
                return final
        ...
        for repeat in range(0, 2):
            if isinstance(value, str) and "{" in value:
                try:
                    ...
                    self.record_status(
                        "Warn - can not resolve {} value {}".format(arg, value),
                        had_errors=True,
                    )
                    value = default
        ...
        if indirect and isinstance(value, str) and "." in value:
            ...
            if attribute:
                value = self.get_state(
                    entity_id=value, default=default, attribute=attribute
                )
            else:
    ...
    def get_arg(
        ...
        if isinstance(default, float):
            ...
            try:
                ...
                self.log(
                    "WARN: Return bad float value {} from {} using default {}".format(
                        value, arg, default
                    )
                )
                self.record_status(
                    "Warn - Return bad float value {} from {}".format(value, arg),
                    had_errors=True,
                )
                value = default
            ...
            try:
                ...
                self.log(
                    "WARN: Return bad int value {} from {} using default {}".format(
                        value, arg, default
                    )
                )
                self.record_status(
                    "Warn - Return bad int value {} from {}".format(value, arg),
                    had_errors=True,
                )
                value = default
    ...
    def get_ge_url(self, url, headers, now_utc):
        ...
        if url in self.ge_url_cache:
            ...
            if age.seconds < (30 * 60):
                self.log(
                    "Return cached GE data for {} age {} minutes".format(
                        url, self.dp1(age.seconds / 60)
                    )
                )
                return pdata
        ...
        try:
            ...
            self.record_status(
                "Warn - Error downloading GE data from cloud",
                debug=url,
                had_errors=True,
            )
            return False
    ...
    def download_ge_data(self, now_utc):
        ...
        if not geserial:
            self.log(
                "ERROR: GE Cloud has been enabled but ge_cloud_serial is not set to your serial"
            )
            self.record_status(
                "Warn - GE Cloud has been enabled but ge_cloud_serial is not set to your serial",
                had_errors=True,
            )
            return False
        if not gekey:
            self.log(
                "ERROR: GE Cloud has been enabled but ge_cloud_key is not set to your appkey"
            )
            self.record_status(
                "Warn - GE Cloud has been enabled but ge_cloud_key is not set to your appkey",
                had_errors=True,
            )
            return False
        ...
        while days_prev <= self.max_days_previous:
            ...
            url = "https://api.givenergy.cloud/v1/inverter/{}/data-points/{}?pageSize=1024".format(
                geserial, datestr
            )
            while url:
                ...
                if darray is None:
                    self.log("WARN: Error downloading GE data from url {}".format(url))
                    self.record_status(
                        "Warn - Error downloading GE data from cloud", debug=url
                    )
                    return False
        ...
        self.load_minutes_now = self.load_minutes.get(0, 0) - self.load_minutes.get(
            self.minutes_now, 0
        )
        self.import_today_now = self.import_today.get(0, 0) - self.import_today.get(
            self.minutes_now, 0
        )
        self.export_today_now = self.export_today.get(0, 0) - self.export_today.get(
            self.minutes_now, 0
        )
        self.pv_today_now = self.pv_today.get(0, 0) - self.pv_today.get(
            self.minutes_now, 0
        )
        self.log(
            "Downloaded {} datapoints from GE going back {} days".format(
    ...
    def download_predbat_releases_url(self, url):
        ...
        if url in self.github_url_cache:
            ...
            if age.seconds < (120 * 60):
                self.log(
                    "Using cached GITHub data for {} age {} minutes".format(
                        url, self.dp1(age.seconds / 60)
                    )
                )
                return pdata
        ...
        if data and isinstance(data, list):
            ...
            if this_tag not in PREDBAT_UPDATE_OPTIONS:
                ...
                if self.releases["this"] == self.releases["latest"]:
                    ...
                    latest_version = (
                        self.releases["latest"] + " " + self.releases["latest_name"]
                    )
                    if auto_update:
                        self.log(
                            "Autoupdate: There is an update pending {} - auto update triggered!".format(
                                latest_version
                            )
                        )
                        ...
                        self.log(
                            "Autoupdate: There is an update pending {} - auto update is off".format(
                                latest_version
                            )
                        )
            ...
            self.log(
                "WARN: Unable to download Predbat version information from github, return code: {}".format(
                    data
                )
            )
    ...
    def download_octopus_rates(self, url):
        ...
        if url in self.octopus_url_cache:
            ...
            if age.seconds < (30 * 60):
                self.log(
                    "Return cached octopus data for {} age {} minutes".format(
                        url, self.dp1(age.seconds / 60)
                    )
                )
                return pdata
        ...
        if not pdata:
            ...
            self.record_status(
                "Warn - Unable to download Octopus data from cloud",
                debug=url,
                had_errors=True,
            )
            if url in self.octopus_url_cache:
    ...
    def futurerate_analysis(self):
        ...
        peak_start = datetime.strptime(
            self.get_arg("futurerate_peak_start", "00:00:00"), "%H:%M:%S"
        )
        peak_end = datetime.strptime(
            self.get_arg("futurerate_peak_end", "00:00:00"), "%H:%M:%S"
        )
                    ...
                    if "Columns" in row:
                        for column in row["Columns"]:
                            ...
                            if "-" in cname and "," in cvalue and cname:
                                ...
                                time_date_start = datetime.strptime(
                                    rstart, TIME_FORMAT_NORD
                                )
                                time_date_end = datetime.strptime(
                                    rend, TIME_FORMAT_NORD
                                )
                                ...
                                if (
                                    minutes_start >= peak_start_minutes
                                    and minutes_end <= peak_end_minutes
                                ):
        ...
        if extracted_keys:
            ...
            self.log(
                "Loaded {} datapoints of futurerate analysis".format(
                    len(extracted_keys)
                )
            )
        ...
        for minute in range(
            minute_now_hour, self.forecast_plan_hours * 60 + minute_now_hour, 60
        ):
            if mdata_import.get(minute) or mdata_export.get(minute):
                future_data.append(
                    "{} => {} / {}".format(
                        self.time_abs_str(minute),
                        mdata_import.get(minute),
                        mdata_export.get(minute),
                    )
                )
    ...
    def download_futurerate_data(self, url):
        ...
        if url in self.futurerate_url_cache:
            ...
            if now_since_midnight.seconds > (
                9.5 * 60 * 60
            ) and now_since_midnight.seconds < (11 * 60 * 60):
            ...
            if not needs_update:
                self.log(
                    "Return cached futurerate data for {} age {} minutes".format(
                        url, self.dp1(age.seconds / 60)
                    )
                )
                return pdata
        ...
        if not pdata:
            ...
            self.record_status(
                "Warn - Unable to download futurerate data from cloud",
                debug=url,
                had_errors=True,
            )
            if url in self.octopus_url_cache:
    ...
    def download_futurerate_data_func(self, url):
        ...
        try:
            ...
            self.record_status(
                "Warn - Error downloading futurerate data from cloud",
                debug=url,
                had_errors=True,
            )
            return {}
    ...
    def download_octopus_rates_func(self, url):
        ...
        while url and pages < 3:
            ...
            try:
                ...
                self.record_status(
                    "Warn - Error downloading Octopus data from cloud",
                    debug=url,
                    had_errors=True,
                )
                return {}
    ...
    def load_car_energy(self, now_utc):
        ...
        if "car_charging_energy" in self.args:
            ...
            self.log(
                "Car charging hold {} threshold {}".format(
                    self.car_charging_hold, self.car_charging_threshold * 60.0
                )
            )
        ...
        for entity_id in entity_ids:
            try:
                history = self.get_history(
                    entity_id=entity_id, days=self.max_days_previous
                )
            except (ValueError, TypeError):
                history = []
            ...
            if history:
                ...
                self.record_status(
                    "Error - Unable to fetch history from {}".format(entity_id),
                    had_errors=True,
                )
                raise ValueError
    ...
    def minute_data(
        ...
        if not history:
            self.log(
                "Warning, empty history passed to minute_data, ignoring (check your settings)..."
            )
            return mdata
    ...
    def get_filtered_load_minute(self, data, minute_previous, historical, step=1):
        ...
        for offset in range(0, step):
            if historical:
                load_yesterday_raw += self.get_historical(
                    data, minute_previous + offset
                )
            else:
                load_yesterday_raw += self.get_from_incrementing(
                    data, minute_previous + offset
                )
        ...
        if self.car_charging_hold and self.car_charging_energy:
            ...
            for offset in range(0, step):
                if historical:
                    car_energy += self.get_historical(
                        self.car_charging_energy, minute_previous + offset
                    )
                else:
                    car_energy += self.get_from_incrementing(
                        self.car_charging_energy, minute_previous + offset
                    )
            load_yesterday = max(0, load_yesterday - car_energy)
        elif self.car_charging_hold and (
            load_yesterday >= (self.car_charging_threshold * step)
        ):
            # Car charging hold - ignore car charging in computation based on threshold
            load_yesterday = max(
                load_yesterday - (self.car_charging_rate[0] * step / 60.0), 0
            )
    ...
    def previous_days_modal_filter(self, data):
        ...
        self.log(
            "Historical data totals for days {} are {} - min {}".format(
                self.days_previous, sum_days, min_sum
            )
        )
        ...
        for days in self.days_previous:
            ...
            for minute in range(0, 24 * 60, PREDICT_STEP):
                minute_previous = 24 * 60 - minute + full_days
                if data.get(minute_previous, 0) == data.get(
                    minute_previous + gap_size, 0
                ):
            ...
            if num_gaps > 0:
                ...
                if (average_day == 0) or (num_gaps >= 24 * 60):
                    self.log(
                        "WARN: Historical day {} has no data, unable to fill gaps normally using nominal 24kWh - you should fix your system!".format(
                            days
                        )
                    )
                    average_day = 24.0
                    ...
                    self.log(
                        "WARN: Historical day {} has {} minutes of gap in the data, filled from {} kWh to make new average {} kWh (percent {}%)".format(
                            days,
                            num_gaps,
                            self.dp2(sum_days_id[days]),
                            self.dp2(average_day),
                            self.dp0(real_data_percent * 100.0),
                        )
                    )
    ...
    def record_length(self, charge_window, charge_limit, best_price):
        ...
        if charge_window:
            for window_n in range(0, len(charge_window)):
                if (
                    charge_limit[window_n] > 0
                    and charge_window[window_n]["average"] <= best_price
                ):
        ...
        end_record = min(
            self.forecast_plan_hours * 60 + next_charge_start,
            self.forecast_minutes + self.minutes_now,
        )
    ...
    def record_status(
        self, message, debug="", had_errors=False, notify=False, extra=""
    ):
        """
    ...
    def scenario_summary_title(self, record_time):
        ...
        for minute in range(0, self.forecast_minutes, 60):
            ...
            minute_timestamp = self.midnight_utc + timedelta(
                seconds=60 * minute_absolute
            )
    ...
    def load_today_comparison(
        ...
        for minute in range(0, 24 * 60, step):
            ...
            if minute < minutes_now:
                for offset in range(0, step):
                    import_value_today += self.get_from_incrementing(
                        import_minutes, minutes_now - minute - offset - 1
                    )
            ...
            for offset in range(0, step):
                import_value_pred += self.get_historical(
                    import_minutes, minute - minutes_now + offset
                )
                forecast_value_pred += self.get_from_incrementing(
                    load_forecast, minute + offset, backwards=False
                )
        ...
        if minutes_now >= 180 and actual_total_now >= 1.0 and actual_total_today > 0.0:
            ...
            difference = 1.0 + (
                (actual_total_today - load_total_pred) / actual_total_today
            )
    ...
    def get_cloud_factor(self, minutes_now, pv_data, pv_data10):
        ...
        if self.metric_cloud_enable:
            ...
            self.log(
                "PV Forecast {} kWh and 10% Forecast {} kWh pv cloud factor {}".format(
                    self.dp1(pv_total), self.dp1(pv_total10), pv_factor_round
                )
            )
            return pv_factor
        else:
    ...
    def step_data_history(
        ...
        for minute in range(0, self.forecast_minutes, step):
            ...
            if load_scaling_dynamic:
                scaling_dynamic = load_scaling_dynamic.get(
                    minute_absolute, scaling_dynamic
                )
            ...
            if load_forecast:
                for offset in range(0, step):
                    load_extra += self.get_from_incrementing(
                        load_forecast, minute_absolute, backwards=False
                    )
    ...
    def run_prediction(
        ...
        while minute < self.forecast_minutes:
            ...
            discharge_window_n = self.in_charge_window(
                discharge_window, minute_absolute
            )

            # Add in standing charge, only for the final plan when we save the results
            if (minute_absolute % (24 * 60)) < step and (
                save in ["best", "base", "base10", "best10"]
            ):
            ...
            for car_n in range(0, self.num_cars):
                if car_load[car_n] > 0.0:
                    ...
                    car_load_scale = max(
                        min(
                            car_load_scale,
                            self.car_charging_limit[car_n] - car_soc[car_n],
                        ),
                        0,
                    )
                ...
                if iboost_today_kwh < self.iboost_max_energy:
                    if self.iboost_gas:
                        if self.rate_gas:
                            ...
                            gas_rate = (
                                self.rate_gas.get(minute_absolute, 99)
                                * self.iboost_gas_scale
                            )
                            electric_rate = self.rate_import.get(minute_absolute, 0)
                            if (electric_rate < gas_rate) and (
                                charge_window_n >= 0 or not self.iboost_charging
                            ):
            ...
            pv_ac = min(
                load_yesterday / self.inverter_loss, pv_now, self.inverter_limit * step
            )
            ...
            if self.iboost_enable:
                if iboost_today_kwh < self.iboost_max_energy:
                    if (
                        self.iboost_solar
                        and pv_dc > (self.iboost_min_power * step)
                        and ((soc * 100.0 / self.soc_max) >= self.iboost_min_soc)
                    ):
                ...
                if minute == 0 and save == "best":
                    ...
                    self.log(
                        "IBoost model predicts usage {} in this run period taking total to {} solar {} gas {} charging {}".format(
                            self.dp2(scaled_boost),
                            self.iboost_next,
                            self.iboost_solar,
                            self.iboost_gas,
                            self.iboost_charging,
                        )
                    )
            ...
            if self.set_discharge_freeze:
                ...
                if (discharge_window_n >= 0) and discharge_limits[
                    discharge_window_n
                ] < 100.0:
                    # Freeze mode
            ...
            charge_rate_now_curve = (
                charge_rate_now * self.battery_charge_power_curve.get(soc_percent, 1.0)
            )
            if (
                ...
                if save in ["best", "best10"]:
                    ...
                    charge_rate_now = (
                        self.battery_rate_max_charge_scaled
                    )  # Assume charge becomes enabled here

                # Apply the charging curve
                charge_rate_now_curve = (
                    charge_rate_now
                    * self.battery_charge_power_curve.get(soc_percent, 1.0)
                )
                ...
                battery_draw = -max(
                    min(charge_rate_now_curve * step, charge_limit_n - soc), 0
                )
                battery_state = "f+"
                ...
                if load_yesterday - pv_ac - pv_dc > 0:
                    battery_draw = min(
                        load_yesterday - pv_ac - pv_dc,
                        discharge_rate_now * step,
                        self.inverter_limit * step - pv_ac,
                    )
                    battery_state = "e-"
                else:
                    battery_draw = max(
                        load_yesterday - pv_ac - pv_dc, -charge_rate_now_curve * step
                    )
                    if battery_draw < 0:
                        battery_state = "e+"
                    else:
            ...
            if battery_draw > 0:
                ...
                if soc < reserve_expected:
                    battery_draw -= (
                        (reserve_expected - soc)
                        * self.battery_loss_discharge
                        * self.inverter_loss
                    )
            ...
            if battery_draw < 0:
                ...
                if soc > self.soc_max:
                    battery_draw_dc += (
                        (soc - self.soc_max) / self.battery_loss
                    ) * inverter_loss
                    soc = self.soc_max
                ...
                if soc > self.soc_max:
                    battery_draw_ac += (soc - self.soc_max) / (
                        self.battery_loss * self.inverter_loss
                    )
                    soc = self.soc_max
            ...
            if (soc < self.best_soc_keep) and (soc > self.reserve):
                ...
                if battery_draw > 0:
                    ...
                    metric_keep += (
                        self.rate_import[minute_absolute]
                        * battery_draw
                        * minute_scaling
                    )
                ...
                if keep_diff > 0:
                    metric_keep += (
                        self.rate_import[minute_absolute] * keep_diff * minute_scaling
                    )
            ...
            if (minute % 10) == 0:
                ...
                predict_pv_power[stamp] = self.dp3(
                    (
                        pv_forecast_minute_step[minute]
                        + pv_forecast_minute_step[minute + step]
                    )
                    * (30 / step)
                )
        ...
        if self.debug_enable or save:
            ...
            self.log(
                "    SOC: [{}]".format(
                    self.scenario_summary(record_time, predict_soc_time)
                )
            )
            self.log(
                "  STATE: [{}]".format(
                    self.scenario_summary(record_time, predict_state)
                )
            )
            self.log(
                "   LOAD: [{}]".format(
                    self.scenario_summary(record_time, load_kwh_time)
                )
            )
            self.log(
                "     PV: [{}]".format(self.scenario_summary(record_time, pv_kwh_time))
            )
            self.log(
                " IMPORT: [{}]".format(
                    self.scenario_summary(record_time, import_kwh_time)
                )
            )
            self.log(
                " EXPORT: [{}]".format(
                    self.scenario_summary(record_time, export_kwh_time)
                )
            )
            if self.iboost_enable:
                self.log(
                    " IBOOST: [{}]".format(
                        self.scenario_summary(record_time, predict_iboost)
                    )
                )
            for car_n in range(0, self.num_cars):
                self.log(
                    "   CAR{}: [{}]".format(
                        car_n,
                        self.scenario_summary(record_time, predict_car_soc_time[car_n]),
                    )
                )
            self.log(
                " METRIC: [{}]".format(self.scenario_summary(record_time, metric_time))
            )
        ...
        if save and save == "base" and not SIMULATE:
            ...
            for car_n in range(0, self.num_cars):
                ...
                self.dashboard_item(
                    ...
                    state=self.dp2(
                        final_car_soc[car_n]
                        / self.car_charging_battery_size[car_n]
                        * 100.0
                    ),
                    attributes={
            ...
            self.log(
                "Battery has {} hours left - now at {}".format(
                    self.dp2(hours_left), self.dp2(self.soc_kw)
                )
            )
            self.dashboard_item(
                self.prefix + ".metric",
    ...
    def time_now_str(self):
        ...
        """
        return (self.midnight + timedelta(minutes=self.minutes_now)).strftime(
            "%H:%M:%S"
        )
            ...
            if minute not in rates:
                ...
                if minute_mod not in adjusted_rates:
                    if (
                        ...
                        rate_offset = (
                            rate_offset + self.metric_future_rate_offset_import
                        )
                    elif (not is_import) and (not is_gas):
                        rate_offset = max(
                            rate_offset + self.metric_future_rate_offset_export, 0
                        )
    ...
    def find_charge_window(self, rates, minute, threshold_rate, find_high):
        ...
        while minute < stop_at:
            ...
            if minute >= (self.forecast_minutes + self.minutes_now) and (
                rate_low_start < 0
            ):
                break
            ...
            if minute in rates:
                ...
                if (
                    ...
                    if (
                        ...
                        and (
                            int(self.dp2(rate) + 0.5)
                            != int(self.dp2(rate_low_rate) + 0.5)
                        )
                    ):
                    ...
                    if (
                        (
                            rate_low_start in self.manual_all_times
                            or minute in self.manual_all_times
                        )
                        and (rate_low_start >= 0)
                    ...
                    if (
                        find_high
                        and (rate_low_start >= 0)
                        and ((minute - rate_low_start) >= 60 * 4)
                    ):
        ...
        for this_rate in info:
            if this_rate:
                ...
                try:
                    ...
                    self.log(
                        "WARN: Bad start time {} provided in energy rates".format(
                            start_str
                        )
                    )
                    self.record_status(
                        "Bad start time {} provided in energy rates".format(start_str),
                        had_errors=True,
                    )
                    continue
                ...
                try:
                    ...
                    self.log(
                        "WARN: Bad end time {} provided in energy rates".format(end_str)
                    )
                    self.record_status(
                        "Bad end time {} provided in energy rates".format(end_str),
                        had_errors=True,
                    )
                    continue
                ...
                if "date" in this_rate:
                    ...
                    try:
                        ...
                        self.log(
                            "WARN: Bad date {} provided in energy rates".format(
                                this_rate["date"]
                            )
                        )
                        self.record_status(
                            "Bad date {} provided in energy rates".format(
                                this_rate["date"]
                            ),
                            had_errors=True,
                        )
                ...
                try:
                    ...
                    self.record_status(
                        "Bad rate {} provided in energy rates".format(rate),
                        had_errors=True,
                    )
                    continue
                ...
                self.log(
                    "Adding rate {} => {} to {} @ {} date {}".format(
                        this_rate,
                        self.time_abs_str(start_minutes),
                        self.time_abs_str(end_minutes),
                        rate,
                        date,
                    )
                )
    ...
    def plan_car_charging(self, car_n, low_rates):
        ...
        try:
            ready_time = datetime.strptime(
                self.car_charging_plan_time[car_n], "%H:%M:%S"
            )
        except ValueError:
    ...
    def add_now_to_octopus_slot(self, octopus_slots, now_utc):
        ...
        for car_n in range(0, self.num_cars):
            if self.car_charging_now[car_n]:
                ...
                slot_start_date = self.midnight_utc + timedelta(
                    minutes=minutes_start_slot
                )
        ...
        for octopus_saving_slot in octopus_saving_slots:
            ...
            if start and end:
                try:
                    ...
                    self.log(
                        "WARN: Unable to decode Octopus saving session start/end time"
                    )
            if state and (not start or not end):
                ...
                end_minutes = min(
                    self.minutes_to_time(end, self.midnight_utc), self.forecast_minutes
                )

            if (
                start_minutes >= 0
                and end_minutes != start_minutes
                and start_minutes < self.forecast_minutes
            ):
                self.log(
        ...
        for slot in octopus_slots:
            ...
            if "charge_in_kwh" in slot:
                kwh = abs(
                    float(
                        slot.get(
                            "charge_in_kwh", self.car_charging_rate[0] * slot_hours
                        )
                    )
                )
            else:
                kwh = abs(
                    float(slot.get("chargeKwh", self.car_charging_rate[0] * slot_hours))
                )
    ...
    def publish_car_plan(self):
        ...
        for car_n in range(self.num_cars):
            ...
            if not self.car_charging_slots[car_n]:
                ...
                if (
                    self.minutes_now >= window["start"]
                    and self.minutes_now < window["end"]
                ):
                    slot = True
                else:
                    slot = False
    ...
    def publish_rates_export(self):
        ...
        if self.high_export_rates:
            ...
            for window in self.high_export_rates:
                ...
                window_str += "{}: {} - {} @ {}".format(
                    window_n,
                    self.time_abs_str(rate_high_start),
                    self.time_abs_str(rate_high_end),
                    rate_high_average,
                )

                rate_high_start_date = self.midnight_utc + timedelta(
                    minutes=rate_high_start
                )
                rate_high_end_date = self.midnight_utc + timedelta(
                    minutes=rate_high_end
                )
                ...
                if window_n == 0 and not SIMULATE:
                    ...
                    in_high_rate = (
                        self.minutes_now >= rate_high_start
                        and self.minutes_now <= rate_high_end
                    )
                    self.dashboard_item(
                    ...
                    high_rate_minutes = (
                        (rate_high_end - self.minutes_now)
                        if in_high_rate
                        else (rate_high_end - rate_high_start)
                    )
                    self.dashboard_item(
    ...
    def rate_minmax(self, rates):
        ...
        if rates:
            ...
            for minute in range(
                self.minutes_now, self.forecast_minutes + self.minutes_now
            ):
                if minute in rates:
                    rate = rates[minute]
                    if rate > rate_max:
    ...
    def rate_min_forward_calc(self, rates):
        ...
        for minute in range(
            self.minutes_now, self.forecast_minutes + 24 * 60 + self.minutes_now
        ):
            rate_min_forward[minute] = min(rate_array[minute:])

        self.log(
            "Rate min forward looking: now {} at end of forecast {}".format(
                self.dp2(rate_min_forward[self.minutes_now]),
                self.dp2(rate_min_forward[self.forecast_minutes]),
            )
        )
    ...
    def rate_scan_window(self, rates, rate_low_min_window, threshold_rate, find_high):
        ...
        while True:
            ...
            if rate_low_start >= 0:
                if (
                    rate_low_end > self.minutes_now
                    and (rate_low_end - rate_low_start) >= rate_low_min_window
                ):
        ...
        (
            ...
            price_set,
            price_links,
        ) = self.sort_window_by_price_combined(found_rates, [], stand_alone=True)
        if not find_high:
            price_set.reverse()
        ...
        for loop_price in price_set:
            ...
            while these_items:
                ...
                if window_start >= (self.minutes_now + self.forecast_minutes):
                    ...
                    take_enable
                    or (not find_high and window_start < upcoming_period)
                    or window_start in self.manual_all_times
                ):
    ...
    def set_rate_thresholds(self):
        ...
        if self.rate_low_threshold > 0:
            self.rate_import_cost_threshold = self.dp2(
                self.rate_average * self.rate_low_threshold
            )
        else:
            # In automatic mode select the only rate or everything but the most expensive
            if (self.rate_max == self.rate_min) or (
                self.rate_export_max > self.rate_max
            ):
        ...
        if self.rate_high_threshold > 0:
            self.rate_export_cost_threshold = self.dp2(
                self.rate_export_average * self.rate_high_threshold
            )
        else:
            # In automatic mode select the only rate or everything but the most cheapest
            if (self.rate_export_max == self.rate_export_min) or (
                self.rate_export_min > self.rate_min
            ):
        ...
        if self.rate_export_max == self.rate_export_min:
            self.rate_export_cost_threshold = max(
                self.rate_export_cost_threshold, self.dp2(self.rate_min)
            )

        self.log(
            "Rate thresholds (for charge/discharge) are import {}p ({}) export {}p ({})".format(
                self.rate_import_cost_threshold,
                self.rate_low_threshold,
                self.rate_export_cost_threshold,
                self.rate_high_threshold,
            )
        )
    ...
    def rate_add_io_slots(self, rates, octopus_slots):
        ...
        if octopus_slots:
            ...
            for slot in octopus_slots:
                ...
                if source != "bump-charge":
                    start_minutes = max(
                        self.minutes_to_time(start, self.midnight_utc), 0
                    )
                    end_minutes = max(
                        min(
                            self.minutes_to_time(end, self.midnight_utc),
    ...
    def rate_scan(self, rates, print=True):
        ...
        (
            ...
            self.rate_max,
            self.rate_average,
            self.rate_min_minute,
            self.rate_max_minute,
        ) = self.rate_minmax(rates)
        ...
        if print:
            ...
            self.log(
                "Import rates min {} max {} average {}".format(
                    self.rate_min, self.rate_max, self.rate_average
                )
            )
    ...
    def rate_scan_gas(self, rates, print=True):
        ...
        if print:
            self.log(
                "Gas rates min {} max {} average {}".format(
                    rate_gas_min, rate_gas_max, rate_gas_average
                )
            )
    ...
    def publish_rates_import(self):
        ...
        if self.low_rates:
            ...
            for window in self.low_rates:
                ...
                window_str += "{}: {} - {} @ {}".format(
                    window_n,
                    self.time_abs_str(rate_low_start),
                    self.time_abs_str(rate_low_end),
                    rate_low_average,
                )

                rate_low_start_date = self.midnight_utc + timedelta(
                    minutes=rate_low_start
                )
                ...
                if window_n == 0 and not SIMULATE:
                    ...
                    in_low_rate = (
                        self.minutes_now >= rate_low_start
                        and self.minutes_now <= rate_low_end
                    )
                    self.dashboard_item(
                    ...
                    low_rate_minutes = (
                        (rate_low_end - self.minutes_now)
                        if in_low_rate
                        else (rate_low_end - rate_low_start)
                    )
                    self.dashboard_item(
    ...
    def publish_html_plan(self, pv_forecast_minute_step, load_minutes_step, end_record):
        ...
        for minute in range(minute_now_align, end_plan, 30):
            ...
            for try_minute in range(minute, minute + 30, 5):
                charge_window_n = self.in_charge_window(
                    self.charge_window_best, try_minute
                )
                if charge_window_n >= 0:
                    break
            ...
            if charge_window_n >= 0 and not in_span:
                rowspan = int(
                    (self.charge_window_best[charge_window_n]["end"] - minute) / 30
                )
                if rowspan > 1:
                    in_span = True
                    start_span = True
            ...
            for try_minute in range(minute, minute + 30, 5):
                discharge_window_n = self.in_charge_window(
                    self.discharge_window_best, try_minute
                )
            ...
            for offset in range(0, 30, PREDICT_STEP):
                pv_forecast += pv_forecast_minute_step.get(
                    minute_relative + offset, 0.0
                )
            ...
            soc_percent = int(
                self.dp2(
                    (self.predict_soc_best.get(minute_relative, 0.0) / self.soc_max)
                    * 100.0
                )
                + 0.5
            )
            soc_percent_end = int(
                self.dp2(
                    (
                        self.predict_soc_best.get(minute_relative + 30, 0.0)
                        / self.soc_max
                    )
                    * 100.0
                )
            ...
            soc_change = self.predict_soc_best.get(
                minute_relative + 30, 0.0
            ) - self.predict_soc_best.get(minute_relative, 0.0)
            metric_start = self.predict_metric_best.get(minute_relative, 0.0)
            metric_end = self.predict_metric_best.get(
                minute_relative + 30, metric_start
            )
            ...
            if (
                rate_value_import <= 0
            ):  # colour the import rate, blue for negative, then green, yellow and red
            ...
            if charge_window_n >= 0:
                ...
                if limit > 0.0:
                    if self.set_charge_freeze and (limit == self.reserve):
                        ...
                        state_color = "#3AEE85"
                    if (
                        self.charge_window_best[charge_window_n]["start"]
                        in self.manual_charge_times
                    ):
                        state += " &#8526;"
            ...
            if discharge_window_n >= 0:
                ...
                if (
                    self.discharge_window_best[discharge_window_n]["start"]
                    in self.manual_discharge_times
                ):
                    state += " &#8526;"
            ...
            if plan_debug:
                rate_str_import += " (%02.02f)" % (
                    rate_value_import / self.battery_loss / self.inverter_loss
                    + self.metric_battery_cycle
                )
            ...
            if plan_debug:
                rate_str_export += " (%02.02f)" % (
                    rate_value_export * self.battery_loss_discharge * self.inverter_loss
                    - self.metric_battery_cycle
                )
            ...
            if self.iboost_enable:
                ...
                iboost_change = (
                    self.predict_iboost_best.get(minute_relative + 30, 0)
                    - iboost_amount
                )
            ...
            html += (
                "<td bgcolor=" + pv_color + ">" + str(pv_forecast) + pv_symbol + "</td>"
            )
            ...
            if self.iboost_enable:
                html += (
                    "<td bgcolor=" + iboost_color + ">" + iboost_amount_str + " </td>"
                )
            html += (
                "<td bgcolor=" + soc_color + ">" + str(soc_percent) + soc_sym + "</td>"
            )
    ...
    def publish_discharge_limit(self, discharge_window, discharge_limits, best):
        ...
        if not SIMULATE:
            ...
            if discharge_window and (
                discharge_window[0]["end"] < (24 * 60 + self.minutes_now)
            ):
                ...
                time_format_time = "%H:%M:%S"
                discharge_startt = self.midnight_utc + timedelta(
                    minutes=discharge_start_minutes
                )
                discharge_endt = self.midnight_utc + timedelta(
                    minutes=discharge_end_minutes
                )
    ...
    def publish_charge_limit(
        self, charge_limit, charge_window, charge_limit_percent, best=False, soc={}
    ):
        """
        ...
        if not SIMULATE:
            ...
            if charge_limit:
                ...
                if charge_window[0]["end"] <= (24 * 60 + self.minutes_now):
                    ...
                    time_format_time = "%H:%M:%S"
                    charge_startt = self.midnight_utc + timedelta(
                        minutes=charge_start_minutes
                    )
                    charge_endt = self.midnight_utc + timedelta(
                        minutes=charge_end_minutes
                    )
    ...
    def optimise_charge_limit_price(
        ...
        if region_start:
            self.log(
                "Region {} - {}".format(
                    self.time_abs_str(region_start), self.time_abs_str(region_end)
                )
            )
        window_prices = {}
                ...
                for divide in [1, 2, 3, 4, 8, 16, 96]:
                    ...
                    for discharge_enable in discharge_enable_options:
                        ...
                        for window_n in range(0, record_charge_windows):
                            if region_start:
                                if (
                                    charge_window[window_n]["start"] > region_end
                                    or charge_window[window_n]["end"] < region_start
                                ):
                                    continue
                        ...
                        if discharge_enable:
                            ...
                            for window_n in all_d:
                                if region_start:
                                    if (
                                        discharge_window[window_n]["start"] > region_end
                                        or discharge_window[window_n]["end"]
                                        < region_start
                                    ):
                                        continue
                                ...
                                if (
                                    ...
                                    continue
                                if (
                                    window_prices_discharge[window_n]
                                    < lowest_price_discharge
                                ):
                                    lowest_price_discharge = window_prices_discharge[
                                        window_n
                                    ]
                        ...
                        metric += (
                            battery_cycle * self.metric_battery_cycle + metric_keep
                        )
        ...
        for try_soc in try_socs:
            ...
            if not first_window and not all_n:
                ...
                if try_soc > (max_soc + loop_step):
                    ...
                    continue
                if (
                    (try_soc > self.reserve)
                    and (try_soc > self.best_soc_min)
                    and (try_soc < (min_soc - loop_step))
                ):
                    self.debug_enable = was_debug
                    if self.debug_enable and 0:
                        self.log(
            ...
            if first_window and not all_n:
                ...
                predict_minute_start = max(
                    int((window["start"] - self.minutes_now) / 5) * 5, 0
                )
                predict_minute_end = int((window["end"] - self.minutes_now) / 5) * 5
                if (predict_minute_start in self.predict_soc) and (
                    predict_minute_end in self.predict_soc
                ):
                    min_soc = min(
            ...
            if (
                not all_n
                and (window_n == self.in_charge_window(charge_window, self.minutes_now))
                and (try_soc != self.reserve)
            ):
    ...
    def optimise_discharge(
        ...
        for loop_limit in loop_options:
            ...
            while loop_start >= window["start"]:
                ...
                if (this_discharge_limit in [100.0, 99.0]) and (
                    start != window["start"]
                ):
                    continue
                ...
                this_discharge_limit = max(
                    self.calc_percent_limit(max(self.best_soc_min, self.reserve)),
                ...
                if (
                    window_n < 2
                    and this_discharge_limit < 99.0
                    and self.discharge_window
                ):
                    ...
                    if (
                        ...
                        if (
                            self.minutes_now >= dwindow["start"]
                            and self.minutes_now < dwindow["end"]
                        ) or (dwindow["end"] == pwindow["start"]):
                ...
                if (
                    (metric + self.metric_min_improvement_discharge) <= off_metric
    ...
    def sort_window_by_price_combined(
        self,
        charge_windows,
        discharge_windows,
        stand_alone=False,
        ...
        if self.calculate_best_charge or stand_alone:
            ...
            for window in charge_windows:
                ...
                if stand_alone:
                    ...
                    average = self.dp2(
                        window["average"] / self.inverter_loss / self.battery_loss
                        + self.metric_battery_cycle
                    )
                if secondary_order:
                    average_export = (
                        self.rate_export.get(window["start"], 0)
                        + self.rate_export.get(window["end"], 0)
                        - 5
                    ) / 2
                else:
                    average_export = 0
                sort_key = "%04.2f_%04.2f_%03d_c%02d" % (
                    5000 - average,
                    5000 - average_export,
                    999 - id,
                    id,
                ...
                window_links[sort_key]["average"] = self.dp1(
                    average
                )  # Round to nearest 0.1 penny to avoid too many bands
                id += 1
        ...
        if self.calculate_best_discharge and not stand_alone:
            ...
            for window in discharge_windows:
                ...
                average = self.dp2(
                    window["average"] * self.inverter_loss * self.battery_loss_discharge
                    - self.metric_battery_cycle
                )
                if secondary_order:
                    average_import = (
                        self.rate_import.get(window["start"], 0)
                        + self.rate_import.get(window["end"], 0)
                        - 5
                    ) / 2
                else:
                    average_import = 0
                sort_key = "%04.2f_%04.2f_%03d_d%02d" % (
                    5000 - average,
                    5000 - average_import,
                    999 - id,
                    id,
    ...
    def discard_unused_charge_slots(
        self, charge_limit_best, charge_window_best, reserve
    ):
        """
    ...
    def find_spare_energy(self, predict_soc, predict_export, step, first_charge):
        ...
        for trigger in triggers:
            ...
            try:
                ...
                energy = 0.0
                self.log(
                    "WARN: Bad energy value {} provided via trigger {}".format(
                        energy, name
                    )
                )
                self.record_status(
                    "ERROR: Bad energy value {} provided via trigger {}".format(
                        energy, name
                    ),
                    had_errors=True,
                )
            ...
            if total_energy >= energy:
                ...
                state = "off"
            self.log(
                "Evaluate trigger {} results {} total_energy {}".format(
                    trigger, state, self.dp2(total_energy)
                )
            )
            self.dashboard_item(
                sensor_name,
    ...
    def set_charge_discharge_status(self, isCharging, isDischarging):
        ...
        self.dashboard_item(
            ...
            attributes={
                "friendly_name": "Predbat is charging",
        ...
        self.dashboard_item(
            ...
            attributes={
                "friendly_name": "Predbat is discharging",
    ...
    def clip_charge_slots(
        ...
        for window_n in range(0, min(record_charge_windows, len(charge_window_best))):
            ...
            if limit <= 0.0:
                ...
                if (predict_minute_start in predict_soc) and (
                    predict_minute_end in predict_soc
                ):
                    ...
                    if (
                        soc_min_percent
                        > self.calc_percent_limit(charge_limit_best[window_n])
                    ) and (charge_limit_best[window_n] != self.reserve):
                ...
                self.log(
                    "WARN: Clip charge window {} as it's already passed".format(
                        window_n
                    )
                )
                charge_limit_best[window_n] = self.best_soc_min
    ...
    def clip_discharge_slots(
        ...
        for window_n in range(
            0, min(record_discharge_windows, len(discharge_window_best))
        ):
            ...
            if limit == 100:
                ...
                if (predict_minute_start in predict_soc) and (
                    ...
                    if soc_min > limit_soc:
                        ...
                        limit_soc = max(
                            limit_soc,
                            soc_min - 10 * self.battery_rate_max_discharge_scaled,
                        )
                        discharge_limits_best[window_n] = self.calc_percent_limit(
                            limit_soc
                        )
                            ...
                            if self.debug_enable:
                                self.log(
                                    "Clip up discharge window {} from {} - {} from limit {} to new limit {}".format(
                                        ...
                                        window_end,
                                        limit,
                                        discharge_limits_best[window_n],
                                    )
                                )
                        ...
                        if self.set_discharge_freeze:
                            ...
                            limit_soc = min(
                                limit_soc,
                                soc_max + 5 * self.battery_rate_max_discharge_scaled,
                            )
                            discharge_limits_best[window_n] = self.calc_percent_limit(
                                limit_soc
                            )
                ...
                self.log(
                    "WARN: Clip discharge window {} as it's already passed".format(
                        window_n
                    )
                )
    ...
    def discard_unused_discharge_slots(
        self, discharge_limits_best, discharge_window_best
    ):
        """
        ...
        for window_n in range(0, len(discharge_limits_best)):
            if discharge_limits_best[window_n] < 100.0:
                ...
                if (
                    new_best
                    and (
                        discharge_window_best[window_n]["start"] == new_best[-1]["end"]
                    )
                    and (discharge_limits_best[window_n] == new_enable[-1])
                    and (
                        discharge_window_best[window_n]["start"]
                        not in self.manual_all_times
                    )
    ...
    def tweak_plan(
        ...
        record_charge_windows = max(
            self.max_charge_windows(
                end_record + self.minutes_now, self.charge_window_best
            ),
            1,
        )
        record_discharge_windows = max(
            self.max_charge_windows(
                end_record + self.minutes_now, self.discharge_window_best
            ),
            1,
        )
        ...
        window_sorted, window_index = self.sort_window_by_time_combined(
            ...
            self.discharge_window_best[:record_discharge_windows],
        )
        for key in window_sorted:
            ...
            if typ == "c":
                ...
                if self.calculate_best_charge and (
                    window_start not in self.manual_all_times
                ):
                    (
                        best_soc,
                        best_metric,
                ...
                if self.calculate_best_discharge and (
                    window_start not in self.manual_all_times
                ):
        ...
        self.log(
            "Tweak optimisation finished metric {} cost {} metric_keep {}".format(
                self.dp2(best_metric), self.dp2(best_cost), self.dp2(best_keep)
            )
        )
    ...
    def optimise_all_windows(
        ...
        record_charge_windows = max(
            self.max_charge_windows(
                self.end_record + self.minutes_now, self.charge_window_best
            ),
            1,
        )
        record_discharge_windows = max(
            self.max_charge_windows(
                self.end_record + self.minutes_now, self.discharge_window_best
            ),
            1,
        )
        (
        ...
        if price_set and self.calculate_best_charge and self.charge_window_best:
            ...
            if self.calculate_regions:
                self.end_record = self.record_length(
                    self.charge_window_best, self.charge_limit_best, best_price
                )
                ...
                for key in links:
                    ...
                    if typ == "c":
                        ...
                        if (
                            ...
                            and (self.charge_limit_best[window_n] != self.soc_max)
                        ):
                            if self.debug_enable:
                                self.log(
                                    "Skip start at high window {} best limit {}".format(
                                        window_n, (self.charge_limit_best[window_n])
                                    )
                                )
                            continue
                            ...
                            if self.debug_enable:
                                self.log(
                                    "Best charge limit window {} time {} - {} cost {} charge_limit {} (adjusted) min {} @ {} (margin added {} and min {} max {}) with metric {} cost {} windows {}".format(
                                        window_n,
                                        self.time_abs_str(
                                            self.charge_window_best[window_n]["start"]
                                        ),
                                        self.time_abs_str(
                                            self.charge_window_best[window_n]["end"]
                                        ),
                                        average,
                                        self.dp2(best_soc),
                                        self.dp2(soc_min),
                        ...
                        if start_at_low and (
                            (price > best_price)
                            or (self.discharge_limits_best[window_n] == 100.0)
                        ):
                            continue
                        ...
                        if self.calculate_best_discharge and (
                            ...
                            if not self.calculate_discharge_oncharge:
                                ...
                                if (
                                    hit_charge >= 0
                                    and self.charge_limit_best[hit_charge] > 0.0
                                ):
                                    continue
                            ...
                            if self.debug_enable:
                                self.log(
                                    "Best discharge limit window {} time {} - {} cost {} discharge_limit {} (adjusted) min {} @ {} (margin added {} and min {}) with metric {} cost {}".format(
                                        window_n,
                                        self.time_abs_str(
                                            self.discharge_window_best[window_n][
                                                "start"
                                            ]
                                        ),
                                        self.time_abs_str(
    ...
    def optimise_charge_windows_manual(self):
        ...
        if self.discharge_window_best and self.calculate_best_discharge:
            for window_n in range(0, len(self.discharge_window_best)):
                if (
                    self.discharge_window_best[window_n]["start"]
                    in self.manual_idle_times
                ):
    ...
    def optimise_charge_windows_reset(self, reset_all):
        ...
        if self.charge_window_best and self.calculate_best_charge:
            # Set all to max
            for window_n in range(0, len(self.charge_window_best)):
                if self.charge_window_best[window_n]["start"] < (
                    self.minutes_now + self.end_record
                ):
                    if reset_all:
    ...
    def get_car_charging_planned(self):
        ...
        self.car_charging_now_response = self.get_arg(
        ...
        for car_n in range(0, self.num_cars):
            ...
            self.car_charging_plan_smart[car_n] = self.get_arg(
                "car_charging_plan_smart", False
            )
            self.car_charging_plan_time[car_n] = self.get_arg(
            ...
            self.car_charging_limit[car_n] = (
                float(self.get_arg("car_charging_limit", 100.0, index=car_n))
                * self.car_charging_battery_size[car_n]
            ) / 100.0
    ...
    def fetch_pv_datapoints(self, argname):
        ...
        if argname in self.args:
            ...
            if entity_id:
                ...
                try:
                    data = self.get_state(
                        entity_id=self.get_arg(argname, indirect=False),
                        attribute=attribute,
                    )
                except (ValueError, TypeError):
                    self.log(
    ...
    def balance_inverters(self):
        ...
        self.minutes_now = (
            int((now - self.midnight).seconds / 60 / PREDICT_STEP) * PREDICT_STEP
        )
        ...
        for inverter in inverters:
            soc_low.append(
                ...
                and (
                    abs(inverter.soc_percent - soc_max)
                    >= self.balance_inverters_discharge
                )
            )
            soc_high.append(
                ...
                and (
                    abs(inverter.soc_percent - soc_min) >= self.balance_inverters_charge
                )
            )
        ...
        for id in range(0, num_inverters):
            ...
            can_power_house.append(
                (total_discharge_rates - discharge_rates[id] - 200)
                >= total_battery_power
            )
            can_store_pv.append(
                total_pv_power <= (total_charge_rates - charge_rates[id])
            )
        ...
        for this_inverter in range(0, num_inverters):
            ...
            if (
                ...
                and (
                    power_enough_discharge[this_inverter]
                    or discharge_rates[this_inverter] == 0
                )
            ):
                self.log(
                ...
                and (
                    power_enough_charge[this_inverter]
                    or charge_rates[this_inverter] == 0
                )
            ):
                self.log(
        ...
        for id in range(0, num_inverters):
            if (
                not balance_reset_charge.get(id, False)
                and total_charge_rates != 0
                and charge_rates[id] == 0
            ):
                self.log(
                ...
                inverters[id].adjust_charge_rate(
                    inverter.battery_rate_max_charge * 60 * 1000, notify=False
                )
            if (
                not balance_reset_discharge.get(id, False)
                and total_discharge_rates != 0
                and discharge_rates[id] == 0
            ):
                self.log(
                ...
                inverters[id].adjust_discharge_rate(
                    inverter.battery_rate_max_discharge * 60 * 1000, notify=False
                )
            ...
            while rate_w >= 400:
                ...
                if rate >= min_rate:
                    ...
                    for minute in range(0, minutes_left, PREDICT_STEP):
                        ...
                        rate_scale = (
                            self.battery_charge_power_curve.get(charge_now_percent, 1.0)
                            * self.battery_rate_max_scaling
                        )
                        charge_amount = (
                            rate * rate_scale * PREDICT_STEP * self.battery_loss
                        )
    ...
    def log_option_best(self):
        ...
        opts += "calculate_discharge_oncharge({}) ".format(
            self.calculate_discharge_oncharge
        )
        opts += "set_discharge_freeze_only({}) ".format(self.set_discharge_freeze_only)
        opts += "set_discharge_during_charge({}) ".format(
        ...
        opts += "battery_loss_discharge ({} %) ".format(
            int((1 - self.battery_loss_discharge) * 100.0)
        )
        opts += "inverter_hybrid({}) ".format(self.inverter_hybrid)
        opts += "metric_min_improvement({} p) ".format(self.metric_min_improvement)
        opts += "metric_min_improvement_discharge({} p) ".format(
            self.metric_min_improvement_discharge
        )
        opts += "metric_battery_cycle({} p/kWh)".format(self.metric_battery_cycle)
        opts += "metric_battery_value_scaling({} x)".format(
            self.metric_battery_value_scaling
        )
    ...
    def calculate_plan(self, recompute=True):
        ...
        if recompute:
            ...
            self.charge_limit_best = [
                self.current_charge_limit * self.soc_max / 100.0
                for i in range(0, len(self.charge_window_best))
            ]
            self.charge_limit_percent_best = [
                self.current_charge_limit
                for i in range(0, len(self.charge_window_best))
            ...
            self.discharge_limits_best = [
                100.0 for i in range(0, len(self.discharge_window_best))
            ]

            self.end_record = self.forecast_minutes
        # Show best windows
        self.log(
            "Best charge    window {}".format(
                self.window_as_text(self.charge_window_best, self.charge_limit_best)
            )
        )
        self.log(
            "Best discharge window {}".format(
                self.window_as_text(
                    self.discharge_window_best, self.discharge_limits_best
                )
            )
        )
        ...
        pv_forecast_minute_step = self.step_data_history(
            ...
            self.minutes_now,
            forward=True,
            cloud_factor=self.metric_cloud_coverage,
        )
        ...
        if self.calculate_best and recompute:
            ...
            (
                self.charge_limit_best,
                self.charge_window_best,
            ) = self.remove_intersecting_windows(
                self.charge_limit_best,
                self.charge_window_best,
                self.discharge_limits_best,
                self.discharge_window_best,
            ...
            if self.calculate_best_discharge and self.discharge_window_best:
                ...
                (
                    self.discharge_limits_best,
                    self.discharge_window_best,
                ) = self.discard_unused_discharge_slots(
                ...
                if self.discharge_window_best:
                    ...
                    record_discharge_windows = max(
                        self.max_charge_windows(
                            self.end_record + self.minutes_now,
                            self.discharge_window_best,
                        ),
                        1,
                    )
            ...
            if self.calculate_best_charge and self.charge_window_best:
                ...
                if self.set_charge_window:
                    ...
                    (
                        self.charge_limit_best,
                        self.charge_window_best,
                    ) = self.discard_unused_charge_slots(
                ...
                if self.set_charge_window:
                    ...
                    self.charge_limit_percent_best = self.calc_percent_limit(
                        self.charge_limit_best
                    )
                    self.log(
                        "Filtered charge windows {} reserve {}".format(
                            self.window_as_text(
                                self.charge_window_best, self.charge_limit_percent_best
                            ),
                            self.reserve,
                        )
                    )
        ...
        if self.calculate_best:
            ...
            self.discharge_limits_best = [
                self.dp0(elem) for elem in self.discharge_limits_best
            ]
            ...
            self.publish_charge_limit(
                ...
                self.charge_limit_percent_best,
                best=True,
                soc=self.predict_soc_best,
            )
            self.publish_discharge_limit(
                self.discharge_window_best, self.discharge_limits_best, best=True
            )

            # HTML data
            self.publish_html_plan(
                pv_forecast_minute_step, load_minutes_step, self.end_record
            )
    ...
    def reset_inverter(self):
        ...
        """
        if not self.set_read_only or (
            self.inverter_needs_reset_force in ["set_read_only"]
        ):
            ...
            for inverter in self.inverters:
                self.log(
                    "Reset inverter settings to safe mode (set_charge_window={} set_discharge_window={} force={})".format(
                        ...
                        self.set_discharge_window,
                        self.inverter_needs_reset_force,
                    )
                )
                if self.set_charge_window or (
                    self.inverter_needs_reset_force in ["set_read_only", "mode"]
                ):
                    inverter.adjust_charge_rate(
                        inverter.battery_rate_max_charge * 60.0 * 1000.0
                    )
                ...
                if (
                    self.set_charge_window
                    or self.set_discharge_window
                    or (self.inverter_needs_reset_force in ["set_read_only", "mode"])
                ):
                    inverter.adjust_reserve(0)
                if self.set_discharge_window or (
                    self.inverter_needs_reset_force in ["set_read_only", "mode"]
                ):
                    inverter.adjust_discharge_rate(
                        inverter.battery_rate_max_discharge * 60 * 1000
                    )
    ...
    def execute_plan(self):
        ...
        for inverter in self.inverters:
            ...
            if (not self.set_discharge_during_charge) or (
                not self.car_charging_from_battery
            ):
            ...
            if self.set_charge_window and self.charge_window_best:
                        ...
                        if self.debug_enable:
                            self.log(
                                "Combine window with next window {}-{}".format(
                                    self.time_abs_str(windows["start"]),
                                    self.time_abs_str(windows["end"]),
                                )
                            )

                # Avoid adjust avoid start time forward when it's already started
                if (inverter.charge_start_time_minutes < self.minutes_now) and (
                ...
                if (minutes_start < self.minutes_now) and (
                    (minutes_end - minutes_start) >= 24 * 60
                ):
                ...
                if ((minutes_start - self.minutes_now) < (24 * 60)) and (
                    minutes_end > self.minutes_now
                ):
                    charge_start_time = self.midnight_utc + timedelta(
                    ...
                    if (
                        ...
                        if not self.set_discharge_during_charge and (
                            inverter.soc_percent >= self.charge_limit_percent_best[0]
                        ):
                            inverter.adjust_discharge_rate(0)
                            resetDischarge = False

                        if self.set_charge_freeze and (
                            self.charge_limit_best[0] == self.reserve
                        ):
                            if (
                                self.set_soc_enable
                    ...
                    if not disabled_charge_window:
                        ...
                        if (self.minutes_now < minutes_end) and (
                            (minutes_start - self.minutes_now)
                            <= self.set_window_minutes
                            or (inverter.charge_start_time_minutes - self.minutes_now)
                            <= self.set_window_minutes
                        ):
                            if (
                                (minutes_start - self.minutes_now)
                                > self.set_window_minutes
                            ) and (minutes_end - self.minutes_now) >= 24 * 60:
                                self.log(
                                    "Charge window would wrap, disabling until later"
                                )
                                inverter.disable_charge_window()
                            else:
                                ...
                                inverter.adjust_charge_window(
                                    charge_start_time, charge_end_time, self.minutes_now
                                )
                        else:
                            self.log(
                ...
                elif ((minutes_start - self.minutes_now) >= (24 * 60)) and (
                    inverter.charge_start_time_minutes - self.minutes_now
                ) <= self.set_window_minutes:
                    # No charging require in the next 24 hours
                    self.log(
                        "No charge window required for 24-hours, disabling before the start"
                    )
                    inverter.disable_charge_window()
                else:
                ...
                self.set_charge_window
                and (inverter.charge_start_time_minutes - self.minutes_now)
                <= self.set_window_minutes
            ):
                # No charge windows
            ...
            if self.set_discharge_window and self.discharge_window_best:
                ...
                # Avoid adjust avoid start time forward when it's already started
                if (inverter.discharge_start_time_minutes < self.minutes_now) and (
                    self.minutes_now >= minutes_start
                ):
                    self.log(
                ...
                discharge_start_time = self.midnight_utc + timedelta(
                    minutes=minutes_start
                )
                ...
                if (
                    ...
                    if not self.set_discharge_freeze_only and (
                        ...
                        self.log(
                            "Discharging now - current SOC {} and target {}".format(
                                self.soc_kw, self.dp2(discharge_soc)
                            )
                        )
                        inverter.adjust_discharge_rate(
                            inverter.battery_rate_max_discharge * 60 * 1000
                        )
                        inverter.adjust_force_discharge(
                            True, discharge_start_time, discharge_end_time
                        )
                        resetDischarge = False
                        isDischarging = True
                    ...
                    if (
                        ...
                        inverter.adjust_force_discharge(
                            False, discharge_start_time, discharge_end_time
                        )
                        resetReserve = True
                    else:
                        self.log(
                    ...
                    if self.set_discharge_freeze:
                        ...
                        inverter.adjust_charge_rate(
                            inverter.battery_rate_max_charge * 60 * 1000
                        )
                    ...
                    if self.car_charging_slots[car_n]:
                        ...
                        if (
                            ...
                            if status not in ["Discharging", "Charging"]:
                                ...
                                resetDischarge = False
                                self.log(
                                    "Disabling battery discharge while the car {} is charging".format(
                                        car_n
                                    )
                                )
                                if status != "Idle":
            ...
            if self.set_soc_enable:
                if (
                    ...
                    if self.set_charge_freeze and (
                        ...
                        self.log(
                            "Within charge freeze setting target soc to current soc {}".format(
                                inverter.soc_percent
                            )
                        )
                        inverter.adjust_battery_target(inverter.soc_percent)
                    else:
                        inverter.adjust_battery_target(
                            self.charge_limit_percent_best[0]
                        )
                else:
                        ...
                        if (
                            ...
                            self.log(
                                "Within the charge window, holding SOC setting {} (now {} target set_soc_minutes {} charge start time {})".format(
                                    ...
                                    self.set_soc_minutes,
                                    self.time_abs_str(
                                        inverter.charge_start_time_minutes
                                    ),
                                )
                            )
                        else:
            ...
            if self.set_soc_enable and self.set_reserve_enable and not setReserve:
                if (
                    ...
                    if (inverter.soc_percent > limit) or (not disabled_charge_window):
                        self.log(
                            "Adjust reserve to default as SOC {} % is above target {} % or charging active".format(
                                inverter.soc_percent, limit
                            )
                        )
                        inverter.adjust_reserve(0)
                    else:
                    ...
                    self.log(
                        "Adjust reserve to default (as set_reserve_enable is true)"
                    )
                    inverter.adjust_reserve(0)
                    resetReserve = False
    ...
    def fetch_octopus_rates(self, entity_id, adjust_key=None):
        ...
        if entity_id:
            ...
            if "_current_rate" in entity_id:
                # Try as event
                prev_rate_id = entity_id.replace(
                    "_current_rate", "_previous_day_rates"
                ).replace("sensor.", "event.")
                ...
                if data_import:
                    ...
                    data_import = self.get_state(
                        entity_id=prev_rate_id, attribute="all_rates"
                    )
                    if data_import:
                        data_all += data_import
                    else:
                        self.log(
                            "WARN: No Octopus data in sensor {} attribute 'all_rates'".format(
                                prev_rate_id
                            )
                        )
            ...
            # Current rates
            if "_current_rate" in entity_id:
                current_rate_id = entity_id.replace(
                    "_current_rate", "_current_day_rates"
                ).replace("sensor.", "event.")
            else:
            ...
            if data_import:
                ...
                data_import = self.get_state(
                    entity_id=current_rate_id, attribute="all_rates"
                )
                if data_import:
                    data_all += data_import
                else:
            ...
            # Next rates
            if "_current_rate" in entity_id:
                next_rate_id = entity_id.replace(
                    "_current_rate", "_next_day_rates"
                ).replace("sensor.", "event.")
                ...
                if data_import:
                    ...
                    data_import = self.get_state(
                        entity_id=next_rate_id, attribute="all_rates"
                    )
                    if data_import:
                        data_all += data_import
    ...
    def fetch_sensor_data(self):
            ...
            if "iboost_energy_today" in self.args:
                ...
                if iboost_energy_age >= 1:
                    self.iboost_today = self.dp2(
                        abs(
                            self.iboost_energy_today[0]
                            - self.iboost_energy_today[self.minutes_now]
                        )
                    )
                    self.log(
                        "IBoost energy today from sensor reads {} kWh".format(
                            self.iboost_today
                        )
                    )
        ...
        if self.get_arg("ge_cloud_data", False):
            ...
            if "load_today" in self.args:
                ...
                self.load_minutes_now = max(
                    self.load_minutes.get(0, 0)
                    - self.load_minutes.get(self.minutes_now, 0),
                    0,
                )
            else:
                self.log(
                    "Error: You have not set load_today, you will have no load data"
                )
                self.record_status(
                    message="Error - load_today not set correctly", had_errors=True
                )
                raise ValueError
            ...
            if "import_today" in self.args:
                ...
                self.import_today_now = max(
                    self.import_today.get(0, 0)
                    - self.import_today.get(self.minutes_now, 0),
                    0,
                )
            else:
                self.log(
                    "WARN: You have not set import_today in apps.yaml, you will have no previous import data"
                )
            ...
            if "export_today" in self.args:
                ...
                self.export_today_now = max(
                    self.export_today.get(0, 0)
                    - self.export_today.get(self.minutes_now, 0),
                    0,
                )
            else:
                self.log(
                    "WARN: You have not set export_today in apps.yaml, you will have no previous export data"
                )

            # PV today data
            if "pv_today" in self.args:
                self.pv_today = self.minute_data_import_export(self.now_utc, "pv_today")
                self.pv_today_now = max(
                    self.pv_today.get(0, 0) - self.pv_today.get(self.minutes_now, 0), 0
                )
            else:
                self.log(
                    "WARN: You have not set pv_today in apps.yaml, you will have no previous pv data"
                )
        ...
        if "rates_import_octopus_url" in self.args:
            ...
            self.rate_import = self.download_octopus_rates(
                self.get_arg("rates_import_octopus_url", indirect=False)
            )
            ...
            self.rate_import = self.fetch_octopus_rates(
                entity_id, adjust_key="is_intelligent_adjusted"
            )
            if not self.rate_import:
                self.log(
                    "Error: metric_octopus_import is not set correctly or no energy rates can be read"
                )
                self.record_status(
                    message="Error - metric_octopus_import not set correctly or no energy rates can be read",
                    had_errors=True,
                )
                raise ValueError
        else:
            ...
            self.rate_import = self.basic_rates(
                self.get_arg("rates_import", [], indirect=False), "import"
            )
        ...
        if "metric_octopus_gas" in self.args:
            ...
            if not self.rate_gas:
                self.log(
                    "Error: metric_octopus_gas is not set correctly or no energy rates can be read"
                )
                self.record_status(
                    message="Error - rate_import_gas not set correctly or no energy rates can be read",
                    had_errors=True,
                )
                raise ValueError
            self.rate_gas, self.rate_gas_replicated = self.rate_replicate(
                self.rate_gas, is_import=False, is_gas=False
            )
        ...
        if "octopus_intelligent_slot" in self.args:
            ...
            try:
                completed = self.get_state(
                    entity_id=entity_id, attribute="completedDispatches"
                )
                if not completed:
                    completed = self.get_state(
                        entity_id=entity_id, attribute="completed_dispatches"
                    )
                planned = self.get_state(
                    entity_id=entity_id, attribute="plannedDispatches"
                )
                if not planned:
                    planned = self.get_state(
                        entity_id=entity_id, attribute="planned_dispatches"
                    )
                vehicle = self.get_state(
                    entity_id=entity_id, attribute="registeredKrakenflexDevice"
                )
                vehicle_pref = self.get_state(
                    entity_id=entity_id, attribute="vehicleChargingPreferences"
                )
            except (ValueError, TypeError):
                self.log(
                ...
                self.record_status(
                    message="Error - octopus_intelligent_slot not set correctly",
                    had_errors=True,
                )
            ...
            if self.num_cars >= 1:
                ...
                if vehicle:
                    self.car_charging_battery_size[0] = float(
                        vehicle.get(
                            "vehicleBatterySizeInKwh", self.car_charging_battery_size[0]
                        )
                    )
                    self.car_charging_rate[0] = float(
                        vehicle.get("chargePointPowerInKw", self.car_charging_rate[0])
                    )
                else:
                    size = self.get_state(
                    ...
                    rate = self.get_state(
                        entity_id=entity_id, attribute="charge_point_power_in_kw"
                    )
                    if size:
                ...
                self.car_charging_limit[0] = (
                    float(self.get_arg("car_charging_limit", 100.0, index=0))
                    * self.car_charging_battery_size[0]
                ) / 100.0
                ...
                if vehicle_pref and self.octopus_intelligent_charging:
                    ...
                    octopus_limit = self.dp2(
                        octopus_limit * self.car_charging_battery_size[0] / 100.0
                    )
                    self.car_charging_limit[0] = min(
                        self.car_charging_limit[0], octopus_limit
                    )
                    ...
                    if octopus_limit:
                        try:
                            ...
                            self.log(
                                "Warn: octopus_limit is set to a bad value {} in apps.yaml, must be a number".format(
                                    octopus_limit
                                )
                            )
                            octopus_limit = None
                ...
                if self.octopus_intelligent_charging:
                    self.octopus_slots = self.add_now_to_octopus_slot(
                        self.octopus_slots, self.now_utc
                    )
                    if (
                        not self.octopus_intelligent_ignore_unplugged
                        or self.car_charging_planned[0]
                    ):
                        self.car_charging_slots[0] = self.load_octopus_slots(
                            self.octopus_slots
                        )
                        self.log(
                            "Car 0 using Octopus Intelligent, charging limit {}, ready time {} - battery size {}".format(
                                self.car_charging_limit[0],
                                self.car_charging_plan_time[0],
                                self.car_charging_battery_size[0],
                            )
                        )
                    else:
        ...
        for car_n in range(0, self.num_cars):
            if (car_n == 0) and self.car_charging_manual_soc:
                self.car_charging_soc[car_n] = self.get_arg(
                    "car_charging_manual_soc_kwh"
                )
            else:
                self.car_charging_soc[car_n] = (
                    self.get_arg("car_charging_soc", 0.0, index=car_n)
                    * self.car_charging_battery_size[car_n]
                ) / 100.0
        if self.num_cars:
        ...
        if "rates_export_octopus_url" in self.args:
            ...
            self.rate_export = self.download_octopus_rates(
                self.get_arg("rates_export_octopus_url", indirect=False)
            )
            ...
            if not self.rate_export:
                self.log(
                    "Warning: metric_octopus_export is not set correctly or no energy rates can be read"
                )
                self.record_status(
                    message="Error - metric_octopus_export not set correctly or no energy rates can be read",
                    had_errors=True,
                )
        else:
            # Basic rates defined by user over time
            self.rate_export = self.basic_rates(
                self.get_arg("rates_export", [], indirect=False), "export"
            )
        ...
        if "octopus_saving_session" in self.args:
            ...
            octopoints_per_penny = self.get_arg(
                "octopus_saving_session_octopoints_per_penny", 8
            )  # Default 8 octopoints per found

            entity_id = self.get_arg("octopus_saving_session", indirect=False)
            if entity_id:
                state = self.get_arg("octopus_saving_session", False)

                joined_events = self.get_state(
                    entity_id=entity_id, attribute="joined_events"
                )
                if not joined_events:
                    entity_id = entity_id.replace("binary_sensor.", "event.").replace(
                        "_sessions", "_session_events"
                    )
                    joined_events = self.get_state(
                        entity_id=entity_id, attribute="joined_events"
                    )
                ...
                available_events = self.get_state(
                    entity_id=entity_id, attribute="available_events"
                )
                if available_events:
                    for event in available_events:
                        code = event.get(
                            "code", None
                        )  # decode the available events structure for code, start/end time & rate
                        start = event.get("start", None)
                        end = event.get("end", None)
                        start_time = self.str2time(
                            start
                        )  # reformat the saving session start & end time for improved readability
                        end_time = self.str2time(end)
                        saving_rate = (
                            event.get(
                                "octopoints_per_kwh", saving_rate * octopoints_per_penny
                            )
                            / octopoints_per_penny
                        )  # Octopoints per pence
                        if code:  # Join the new Octopus saving event and send an alert
                            self.log(
                                "Joining Octopus saving event code {} {}-{} at rate {} p/kWh".format(
                                    code,
                                    start_time.strftime("%a %d/%m %H:%M"),
                            ...
                            self.call_notify(
                                "Predbat: Joined Octopus saving event {}-{}, {} p/kWh".format(
                                    start_time.strftime("%a %d/%m %H:%M"),
                                    end_time.strftime("%H:%M"),
                                    saving_rate,
                                )
        ...
        # Standing charge
        self.metric_standing_charge = (
            self.get_arg("metric_standing_charge", 0.0) * 100.0
        )
        ...
        if self.rate_import:
            ...
            self.rate_import = self.rate_add_io_slots(
            ...
            if "rates_import_override" in self.args:
                self.rate_import = self.basic_rates(
                    self.get_arg("rates_import_override", [], indirect=False),
                    "import",
                    self.rate_import,
                )
            ...
            self.record_status(
                message="Error - No import rate data provided", had_errors=True
            )
        ...
        if self.rate_export:
            ...
            self.rate_export, self.rate_export_replicated = self.rate_replicate(
                self.rate_export, is_import=False
            )
            ...
            if "rates_export_override" in self.args:
                self.rate_export = self.basic_rates(
                    self.get_arg("rates_export_override", [], indirect=False),
                    "export",
                    self.rate_export,
                )
            ...
            self.record_status(
                message="Error - No export rate data provided", had_errors=True
            )
        ...
        if self.rate_export:
            self.high_export_rates, lowest, highest = self.rate_scan_window(
                self.rate_export, 5, self.rate_export_cost_threshold, True
            )
            # Update threshold automatically
            self.log(
                "High export rate found rates in range {} to {}".format(lowest, highest)
            )
        ...
        if self.rate_import:
            ...
            self.low_rates, lowest, highest = self.rate_scan_window(
                self.rate_import, 5, self.rate_import_cost_threshold, False
            )
            self.log(
                "Low Import rate found rates in range {} to {}".format(lowest, highest)
            )
            # Update threshold automatically
        ...
        for car_n in range(0, self.num_cars):
            if self.octopus_intelligent_charging and car_n == 0:
                ...
                self.car_charging_slots[car_n] = self.plan_car_charging(
                    car_n, self.low_rates
                )
            else:
                self.log(
                    "Not planning car charging for car {} - car charging planned is False".format(
                        car_n
                    )
                )
            ...
            if self.car_charging_slots[car_n]:
                self.log(
                    "Car {} charging plan is: {}".format(
                        car_n, self.car_charging_slots[car_n]
                    )
                )
        ...
        if self.import_today:
            self.cost_today_sofar = self.today_cost(
                self.import_today, self.export_today
            )
        ...
        self.log(
            "Historical days now {} weight {}".format(
                self.days_previous, self.days_previous_weight
            )
        )
    ...
    def publish_rate_and_threshold(self):
        ...
        if self.rate_export:
            if self.rate_best_cost_threshold_discharge:
                self.rate_export_cost_threshold = (
                    self.rate_best_cost_threshold_discharge
                )
                self.log(
                    "Export threshold used for optimisation was {}p".format(
                        self.rate_export_cost_threshold
                    )
                )
        ...
        if self.rate_import:
            if self.rate_best_cost_threshold_charge:
                ...
                self.log(
                    "Import threshold used for optimisation was {}p".format(
                        self.rate_import_cost_threshold
                    )
                )
    ...
    def fetch_inverter_data(self):
        ...
        for id in range(0, self.num_inverters):
            ...
            if not found_first:
                ...
                if not inverter.inv_support_discharge_freeze:
                    ...
                    self.log(
                        "Note: Inverter does not support discharge freeze - disabled"
                    )
            ...
            self.battery_rate_max_charge_scaled += (
                inverter.battery_rate_max_charge_scaled
            )
            self.battery_rate_max_discharge_scaled += (
                inverter.battery_rate_max_discharge_scaled
            )
        ...
        self.reserve_current_percent = int(
            self.reserve_current / self.soc_max * 100.0 + 0.5
        )
        ...
        self.charge_limit = [
            self.current_charge_limit * self.soc_max / 100.0
            for i in range(0, len(self.charge_window))
        ]
        self.charge_limit_percent = self.calc_percent_limit(self.charge_limit)
        self.publish_charge_limit(
            self.charge_limit, self.charge_window, self.charge_limit_percent, best=False
        )

        self.log(
            "Base charge    window {}".format(
                self.window_as_text(self.charge_window, self.charge_limit)
            )
        )
        self.log(
            "Base discharge window {}".format(
                self.window_as_text(self.discharge_window, self.discharge_limits)
            )
        )
    ...
    def manual_times(self, config_item):
        ...
        for minute in time_overrides:
            minute_str = (self.midnight + timedelta(minutes=minute)).strftime(
                "%H:%M:%S"
            )
    ...
    def fetch_config_options(self):
        ...
        if len(self.days_previous) > len(self.days_previous_weight):
            ...
            self.days_previous_weight += [
                1
                for i in range(
                    0, len(self.days_previous) - len(self.days_previous_weight)
                )
            ]
        if self.holiday_days_left > 0:
            self.days_previous = [1]
            self.log(
                "Holiday mode is active, {} days remaining, setting days previous to 1".format(
                    self.holiday_days_left
                )
            )
        ...
        self.forecast_plan_hours = max(
            min(self.get_arg("forecast_plan_hours", 96), forecast_hours), 8
        )
        ...
        self.inverter_clock_skew_discharge_start = self.get_arg(
            "inverter_clock_skew_discharge_start", 0
        )
        self.inverter_clock_skew_discharge_end = self.get_arg(
        ...
        if (
            ...
            or self.inverter_clock_skew_discharge_end != 0
        ):
            self.log(
                "Inverter clock skew discharge start {} end {} applied".format(
                    self.inverter_clock_skew_discharge_start,
                    self.inverter_clock_skew_discharge_end,
                )
            )

        # Metric config
        self.metric_min_improvement = self.get_arg("metric_min_improvement")
        self.metric_min_improvement_discharge = self.get_arg(
            "metric_min_improvement_discharge"
        )
        self.metric_battery_cycle = self.get_arg("metric_battery_cycle")
        self.metric_future_rate_offset_import = self.get_arg(
            "metric_future_rate_offset_import"
        )
        self.metric_future_rate_offset_export = self.get_arg(
            "metric_future_rate_offset_export"
        )
        ...
        self.battery_charge_power_curve = self.args.get(
        ...
        if not isinstance(self.battery_charge_power_curve, dict):
            ...
            self.record_status(
                "battery_power_curve is incorrectly configured - ignoring",
                had_errors=True,
            )
        ...
        self.octopus_intelligent_ignore_unplugged = self.get_arg(
        ...
        if self.inverter_type != "GE":
            self.log(
                "WARN: Using experimental inverter type {} - not all features are available".format(
                    self.inverter_type
                )
            )
        ...
        self.balance_inverters_crosscharge = self.get_arg(
            "balance_inverters_crosscharge"
        )
        self.balance_inverters_threshold_charge = max(
            self.get_arg("balance_inverters_threshold_charge"), 1.0
        )
        self.balance_inverters_threshold_discharge = max(
            self.get_arg("balance_inverters_threshold_discharge"), 1.0
        )

        if self.set_read_only:
            self.log(
                "NOTE: Read-only mode is enabled, the inverter controls will not be used!!"
            )
        ...
        self.car_charging_threshold = (
            float(self.get_arg("car_charging_threshold")) / 60.0
        )
    ...
    def update_pred(self, scheduled=True):
        ...
        if not scheduled or not self.plan_valid:
            ...
            self.log(
                "Plan was last updated on {} and is now {} minutes old".format(
                    self.plan_last_updated, self.dp1(plan_age_minutes)
                )
            )
        ...
        if not recompute:
            ...
            if (plan_age_minutes + RUN_EVERY) > self.calculate_plan_every:
                ...
                self.log(
                    "Will not recompute the plan, it is {} minutes old and max age is {} minutes".format(
                        self.dp1(plan_age_minutes), self.calculate_plan_every
                    )
                )
        ...
        if scheduled:
            for car_n in range(0, self.num_cars):
                if (car_n == 0) and self.car_charging_manual_soc:
                    self.log(
                        "Car charging Manual SOC current is {} next is {}".format(
                            self.car_charging_soc[car_n],
                            self.car_charging_soc_next[car_n],
                        )
                    )
        ...
        if self.had_errors:
            self.log(
                "Completed run status {} with Errors reported (check log)".format(
                    status
                )
            )
        else:
    ...
    def download_predbat_version(self, version):
        ...
        if tag_split:
            tag = tag_split[0]
            url = (
                "https://raw.githubusercontent.com/springfall2008/batpred/"
                + tag
            ...
            if r.ok:
                ...
                if size >= 10000:
                    self.log(
                        "Write new version {} bytes to {}".format(
                            len(data), new_filename
                        )
                    )
    ...
    def select_event(self, event, data, kwargs):
        ...
        for item in CONFIG_ITEMS:
            if ("entity" in item) and (item["entity"] in entities):
                entity = item["entity"]
                self.log(
                    "select_event: {}, {} = {}".format(item["name"], entity, value)
                )
                if item["name"] == "update":
    ...
    def expose_config(self, name, value, quiet=True, event=False):
        ...
        if item:
            ...
            if not enabled:
                self.log(
                    "Not updating HA config {} to {} as disabled".format(name, value)
                )
                item["value"] = None
            else:
                ...
                if entity and ((item.get("value") is None) or (value != item["value"])):
                    if item.get("reset_inverter", False):
                        ...
                        self.log(
                            "Set reset inverter true due to reset_inverter on item {}".format(
                                name
                            )
                        )
                    if item.get("reset_inverter_force", False):
                        self.inverter_needs_reset = True
                        self.log(
                            "Set reset inverter true due to reset_inverter_force on item {}".format(
                                name
                            )
                        )
                        if event:
                            ...
                            self.log(
                                "Set reset inverter force true due to reset_inverter_force on item {}".format(
                                    name
                                )
                            )
                    item["value"] = value
    ...
    def user_config_item_enabled(self, item):
        ...
        if enable:
            ...
            if citem:
                ...
                self.log(
                    "WARN: Badly formed CONFIG enable item {}, please raise a Github ticket".format(
                        item["name"]
                    )
                )
        return True
    ...
    def restore_settings_yaml(self, filename):
        ...
        if not filename:
            ...
            for item in CONFIG_ITEMS:
                if (item["value"] != item.get("default", None)) and item.get(
                    "restore", True
                ):
                    self.log(
                        "Restore setting: {} = {} (was {})".format(
                            item["name"], item["default"], item["value"]
                        )
                    )
                ...
                with open(filepath, "r") as file:
                    ...
                    for item in settings:
                        ...
                        if (
                            ...
                            and current.get("restore", True)
                        ):
                            self.log(
                                "Restore setting: {} = {} (was {})".format(
                                    item["name"], item["value"], current["value"]
                                )
                            )
    ...
    def create_entity_list(self):
        ...
        if filename:
            ...
            self.log(
                "Failed to write predbat dashboard as can not find config root in {}".format(
                    CONFIG_ROOTS
                )
            )
    ...
    def load_user_config(self, quiet=True, register=False):
            ...
            if not enabled:
                ...
                if ha_value is not None:
                    self.set_state(
                        ...
                        state=ha_value,
                        attributes={
                            "friendly_name": "[Disabled] " + item["friendly_name"]
                        },
                    )
                continue
        ...
        if register:
            ...
            self.fire_event(
                "service_registered", domain="input_number", service="increment"
            )
            self.fire_event(
                "service_registered", domain="input_number", service="decrement"
            )
            ...
            self.fire_event(
                "service_registered", domain="select", service="select_option"
            )
            self.fire_event(
                "service_registered", domain="select", service="select_first"
            )
            self.fire_event(
                "service_registered", domain="select", service="select_last"
            )
            self.fire_event(
                "service_registered", domain="select", service="select_next"
            )
            self.fire_event(
                "service_registered", domain="select", service="select_previous"
            )
    ...
    def resolve_arg_re(self, arg, arg_value, state_keys):
        ...
        if isinstance(arg_value, list):
            ...
            for item_value in arg_value:
                item_matched, item_value = self.resolve_arg_re(
                    arg, item_value, state_keys
                )
                if not item_matched:
                    self.log(
            ...
            for key in state_keys:
                ...
                if res:
                    if len(res.groups()) > 0:
                        self.log(
                            "Regular expression argument {} matched {} with {}".format(
                                arg, my_re, res.group(1)
                            )
                        )
    ...
    def auto_config(self):
        ...
        for arg in self.args:
            ...
            if not matched:
                self.log(
                    "WARN: Regular expression argument: {} unable to match {}, now will disable".format(
                        arg, arg_value
                    )
                )
                disabled.append(arg)
    ...
    def sanity(self):
        ...
        for root in CONFIG_ROOTS:
            if os.path.exists(root):
                config_dir = root
                self.log(
                    "Sanity scan files in '{}' : {}".format(
                        config_dir, os.listdir(config_dir)
                    )
                )
                break
        ...
        if not config_dir:
            self.log(
                "WARN: Unable to find config directory in roots {}".format(CONFIG_ROOTS)
            )
            passed = False
        ...
        if config_dir and os.path.exists(appdaemon_config):
            with open(appdaemon_config, "r") as han:
                ...
                try:
                    ...
                    self.log(
                        "ERROR: Unable to read /config/appdaemon.yaml file correctly!"
                    )
                    passed = False
        ...
        for dir in app_dirs:
            for root, dirs, files in os.walk(dir):
                for name in files:
                    ...
                    if name == "apps.yaml":
                        self.log(
                            "Sanity: Got apps.yaml in location {}".format(filepath)
                        )
                        apps_yaml.append(filepath)
                    elif name == "predbat.py":
                        self.log(
                            "Sanity: Got predbat.py in location {}".format(filepath)
                        )
                        predbat_py.append(filepath)

        if not apps_yaml:
            self.log(
                "WARN: Unable to find any apps.yaml files, please check your configuration"
            )
            passed = False
        ...
        for filename in apps_yaml:
            with open(filename, "r") as han:
                ...
                try:
                    ...
                    self.log(
                        "ERROR: Unable to read {} file correctly!".format(filename)
                    )
                    passed = False
                if data:
                    if "pred_bat" in data:
                        self.log(
                            "Sanity: {} is a valid pred_bat configuration".format(
                                filename
                            )
                        )
                        validPred += 1
        ...
        if not predbat_py:
            ...
            self.log(
                "WARN: Found multiple predbat.py files, please check your configuration"
            )
            passed = False
        else:
                    ...
                    if "THIS_VERSION" in line:
                        ...
                        if res:
                            ...
                            if version != THIS_VERSION:
                                self.log(
                                    "WARN: The version in predbat.py is {} but this code is version {} - please re-start appdaemon".format(
                                        version, THIS_VERSION
                                    )
                                )
                                passed = False
                                ...
                                self.log(
                                    "Sanity: Confirmed correct version {} is in predbat.py".format(
                                        version
                                    )
                                )
                                foundVersion = True
            if not foundVersion:
                self.log(
                    "WARN: Unable to find THIS_VERSION in Predbat.py file, please check your setup"
                )
                passed = False
        ...
        if passed:
            ...
            self.record_status(
                "WARN: Sanity startup checked has FAILED, see your logs for details"
            )
        return passed
    ...
    def initialize(self):
        ...
        self.inverter_type = self.get_arg("inverter_type", "GE", indirect=False)
        self.log(
            f"Inverter Type: {self.inverter_type} ({INVERTER_TYPES[self.inverter_type]})"
        )
        ...
        if SIMULATE and SIMULATE_LENGTH:
            ...
            for offset in range(0, SIMULATE_LENGTH, 30):
                ...
                self.log(
                    ">>>>>>>>>> Simulated offset {} soc {} <<<<<<<<<<<<".format(
                        self.simulate_offset, self.sim_soc_kw
                    )
                )
            ...
            self.log(
                "Predbat: Next run time will be {} and then every {} seconds".format(
                    next_time, run_every
                )
            )
            ...
            if run_every_balance > 0:
                self.log(
                    "Balance inverters will run every {} seconds (if enabled)".format(
                        run_every_balance
                    )
                )
    ...
    def run_time_loop_balance(self, cb_args):
        ...
        """
        if (
            not self.prediction_started
            and self.balance_inverters_enable
            and not self.set_read_only
        ):
            try:
